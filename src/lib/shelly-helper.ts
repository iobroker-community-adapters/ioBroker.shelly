import * as colorconv from './colorconv';
import type { ShellyClient } from './deviceTypes';

/** Base64-encoded SVG icons (Font Awesome) used for generated states in the admin UI. */
const icons: Record<string, string> = {
    // https://fontawesome.com/icons/file-signature?s=solid&f=classic
    signature:
        '',
    // https://fontawesome.com/icons/signal?s=solid&f=classic
    signal: '',
    // https://fontawesome.com/icons/wifi?s=solid&f=classic
    wifi: '',
    // https://fontawesome.com/icons/clock?s=solid&f=classic
    clock: '',
    // https://fontawesome.com/icons/arrows-rotate?s=solid&f=classic
    update: '',
    // https://fontawesome.com/icons/unlock?s=solid&f=classic
    auth: '',
    // https://fontawesome.com/icons/circle-question?s=solid&f=classic
    question:
        '',
    // https://fontawesome.com/icons/microchip?s=solid&f=classic
    chip: '',
};

function getIcon(key: string): string {
    return icons?.[key] ? `data:image/svg+xml;base64,${icons[key]}` : '';
}

/**
 * Celsius to Fahrenheit
 *
 * @param celsius - 10
 */
function celsiusToFahrenheit(celsius: number): number | undefined {
    try {
        const fahrenheit = celsius * 1.8 + 32;
        return Math.round(fahrenheit * 100) / 100;
    } catch {
        return undefined;
    }
}

/**
 * Fahrenheit to Celsius
 *
 * @param fahrenheit - 20
 */
function fahrenheitToCelsius(fahrenheit: number): number | undefined {
    try {
        const celsius = ((fahrenheit - 32) * 5) / 9;
        return Math.round(celsius * 100) / 100;
    } catch {
        return undefined;
    }
}

/**
 *
 * @param self
 * @param name - Name of the device
 */
async function setDeviceName(self: ShellyClient, name: string): Promise<string> {
    const deviceId = self.getDeviceId();
    if (!deviceId) {
        return name;
    }
    const obj = await self.adapter.getObjectAsync(deviceId);

    if (name && obj && obj.common && name !== obj.common.name) {
        await self.adapter.extendObject(deviceId, {
            common: {
                name: name,
            },
        });
    }

    return name;
}

/**
 *
 * @param self
 * @param id - channel id like Relay0
 * @param name - Name of the channel
 */
async function setChannelName(self: ShellyClient, id: string, name: string): Promise<string> {
    const channelId = `${self.getDeviceId()}.${id}`;
    const obj = await self.adapter.getObjectAsync(channelId);

    if (name && obj && obj.common && name !== obj.common.name) {
        await self.adapter.extendObject(channelId, {
            common: {
                name: name,
            },
        });
    }

    return name;
}

/**
 * Get external temperature for a device with a key in unit C or F
 *
 * @param value - like JSON.parse(value)
 * @param key  - '0', '1', ....
 * @param unit . 'C' or 'F'
 */
function getExtTemp(value: any, key: string, unit: string): number | null {
    let unitkey = '';
    switch (unit) {
        case 'C':
            unitkey = 'tC';
            break;
        case 'F':
            unitkey = 'tF';
            break;
        default:
            return 0;
    }
    if (value?.ext_temperature?.[key]?.[unitkey]) {
        return value.ext_temperature[key][unitkey];
    }
    return null;
}

/**
 * Get external humidity for the device with key
 *
 * @param value - like JSON.parse(value)
 * @param key  - '0', '1', ....
 */
function getExtHum(value: any, key: string): number | null {
    if (value?.ext_humidity?.[key]?.hum) {
        return value.ext_humidity[key].hum;
    }
    return null;
}

/**
 *
 * @param self
 */
async function getLightsObjectColor(self: ShellyClient): Promise<Record<string, ioBroker.StateValue | undefined>> {
    const id = self.getDeviceId();
    const obj: Record<string, ioBroker.StateValue | undefined> = {
        ison: 'lights.Switch',
        mode: 'lights.mode',
        red: 'lights.red',
        green: 'lights.green',
        blue: 'lights.blue',
        white: 'lights.white',
        gain: 'lights.gain',
        temp: 'lights.temp',
        brightness: 'lights.brightness',
        effect: 'lights.effect',
    };

    for (const i in obj) {
        const stateId = `${id}.${obj[i]}`;
        const state = await self.adapter.getStateAsync(stateId);
        obj[i] = state ? state.val : undefined;
    }
    return obj;
}

async function getLightsObjectWhite(self: ShellyClient): Promise<Record<string, ioBroker.StateValue | undefined>> {
    const id = self.getDeviceId();
    const obj: Record<string, ioBroker.StateValue | undefined> = {
        ison: 'lights.Switch',
        white: 'lights.white',
        temp: 'lights.temp',
        brightness: 'lights.brightness',
    };

    for (const i in obj) {
        const stateId = `${id}.${obj[i]}`;
        const state = await self.adapter.getStateAsync(stateId);
        obj[i] = state ? state.val : undefined;
    }
    return obj;
}

/**
 * get the hex value for an integer value
 *
 * @param number like 10 or 99
 */
function intToHex(number: number): string {
    if (!number) {
        number = 0;
    }
    let hex = number.toString(16);
    hex = `00${hex}`.slice(-2).toUpperCase(); // 'a' -> '0A'
    return hex;
}

/**
 * get the integer value for a hex value
 *
 * @param hex like 0A or FF
 */
function hextoInt(hex: string): number {
    if (!hex) {
        hex = '00';
    }
    return parseInt(hex, 16);
}

/**
 * get the RGBW value for red, green, blue, white value
 *
 * @param self
 */
async function getRGBW(self: ShellyClient): Promise<string> {
    const id = self.getDeviceId();
    let stateId;
    let state;
    stateId = `${id}.lights.red`;
    state = await self.adapter.getStateAsync(stateId);
    const valred = Number(state ? state.val : 0);
    stateId = `${id}.lights.green`;
    state = await self.adapter.getStateAsync(stateId);
    const valgreen = Number(state ? state.val : 0);
    stateId = `${id}.lights.blue`;
    state = await self.adapter.getStateAsync(stateId);
    const valblue = Number(state ? state.val : 0);
    stateId = `${id}.lights.white`;
    state = await self.adapter.getStateAsync(stateId);
    const valwhite = Number(state ? state.val : 0);
    return `#${intToHex(valred)}${intToHex(valgreen)}${intToHex(valblue)}${intToHex(valwhite)}`;
}

function getColorsFromRGBW(value?: string): { red: number; green: number; blue: number; white: number } {
    value = value || '#00000000';
    const obj = {
        red: hextoInt(value.substr(1, 2) || '00'),
        green: hextoInt(value.substr(3, 2) || '00'),
        blue: hextoInt(value.substr(5, 2) || '00'),
        white: hextoInt(value.substr(7, 2) || '00'),
    };
    return obj;
}

async function getHsvFromRgb(self: ShellyClient): Promise<{ hue: number; saturation: number; brightness: number }> {
    const value = await getRGBW(self);
    const rgbw = getColorsFromRGBW(value);
    const hsv = colorconv.rgbToHsv(rgbw.red, rgbw.green, rgbw.blue);
    return {
        hue: hsv[0],
        saturation: hsv[1],
        brightness: hsv[2],
    };
}

async function getColorsFromHue(self: ShellyClient): Promise<{ red: number; green: number; blue: number }> {
    const id = self.getDeviceId();
    let stateId;
    let state;
    stateId = `${id}.lights.hue`;
    state = await self.adapter.getStateAsync(stateId);
    const valhue = Number(state ? state.val : 0);
    stateId = `${id}.lights.saturation`;
    state = await self.adapter.getStateAsync(stateId);
    const valsaturation = Number(state ? state.val : 0);
    // stateId = id + '.lights.value';
    stateId = `${id}.lights.gain`;
    state = await self.adapter.getStateAsync(stateId);
    const valvalue = Number(state ? state.val : 0);
    const rgb = colorconv.hsvToRgb(valhue, valsaturation, valvalue);
    const obj = {
        red: rgb[0],
        green: rgb[1],
        blue: rgb[2],
    };
    return obj;
}

async function getPowerFactor(self: ShellyClient, channel: number | string): Promise<number> {
    // let stateVoltage = await self.adapter.getStateAsync(self.getDeviceId() + '.Emeter' + channel + '.Voltage');
    const statePower = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter${channel}.Power`);
    const stateReactivePower = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter${channel}.ReactivePower`);
    let pf = 0.0;
    if (statePower && stateReactivePower) {
        // let voltage = stateVoltage.val;
        const power = Number(statePower.val);
        const reactive = Number(stateReactivePower.val);
        if (Math.abs(power) + Math.abs(reactive) > 1.5) {
            pf = power / Math.sqrt(power * power + reactive * reactive);
            pf = Math.round(pf * 100) / 100;
        }
    }
    return pf;
}

/**
 * For EM3, it was not a good idea to implement this function. To far away from a standard
 *
 * @param self
 */
async function getTotalSumm(self: ShellyClient): Promise<number> {
    let calctotal = 0.0;
    const TotalPhase1 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter0.Total`);
    const TotalPhase2 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter1.Total`);
    const TotalPhase3 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter2.Total`);
    calctotal = (TotalPhase1!.val as number) + (TotalPhase2!.val as number) + (TotalPhase3!.val as number);
    calctotal = Math.round(calctotal * 100) / 100;
    return calctotal;
}

async function getTotalReturnedSumm(self: ShellyClient): Promise<number> {
    let calctotal = 0.0;
    const TotalPhase1 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter0.Total_Returned`);
    const TotalPhase2 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter1.Total_Returned`);
    const TotalPhase3 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter2.Total_Returned`);
    calctotal = (TotalPhase1!.val as number) + (TotalPhase2!.val as number) + (TotalPhase3!.val as number);
    calctotal = Math.round(calctotal * 100) / 100;
    return calctotal;
}

/**
 * For EM3, it was not a good idea to implement this function. To far away from a standard
 *
 * @param self
 */
async function getCurrentSumm(self: ShellyClient): Promise<number> {
    let calccurrent = 0.0;
    const CurrentPhase1 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter0.Current`);
    const CurrentPhase2 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter1.Current`);
    const CurrentPhase3 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter2.Current`);
    calccurrent = (CurrentPhase1!.val as number) + (CurrentPhase2!.val as number) + (CurrentPhase3!.val as number);
    calccurrent = Math.round(calccurrent * 100) / 100;
    return calccurrent;
}

/**
 * For EM3, it was not a good idea to implement this function. To far away from a standard
 *
 * @param self
 */
async function getPowerSumm(self: ShellyClient): Promise<number> {
    let calcPower = 0.0;
    const PowerPhase1 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter0.Power`);
    const PowerPhase2 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter1.Power`);
    const PowerPhase3 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter2.Power`);
    calcPower = (PowerPhase1!.val as number) + (PowerPhase2!.val as number) + (PowerPhase3!.val as number);
    calcPower = Math.round(calcPower * 100) / 100;
    return calcPower;
}

/**
 * For EM3, it was not a good idea to implement this function. To far away from a standard
 *
 * @param self
 * @param vtype
 */
async function getVoltageCalc(self: ShellyClient, vtype: string): Promise<number> {
    let calcVoltage = 0.0;
    const VoltagePhase1 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter0.Voltage`);
    const VoltagePhase2 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter1.Voltage`);
    const VoltagePhase3 = await self.adapter.getStateAsync(`${self.getDeviceId()}.Emeter2.Voltage`);
    if (vtype == 'mean') {
        calcVoltage =
            ((VoltagePhase1!.val as number) + (VoltagePhase2!.val as number) + (VoltagePhase3!.val as number)) / 3;
    } else {
        calcVoltage =
            ((VoltagePhase1!.val as number) + (VoltagePhase2!.val as number) + (VoltagePhase3!.val as number)) /
            Math.sqrt(3);
    }
    calcVoltage = Math.round(calcVoltage * 100) / 100;
    return calcVoltage;
}

/**
 * Timer
 *
 * @param self
 * @param id - e.g. 'Relay0.Timer'
 * @param [newVal] - e.g. 10
 */
async function getSetDuration(self: ShellyClient, id: string, newVal?: number): Promise<number | undefined> {
    try {
        id = `${self.getDeviceId()}.${id}`;
        const state = await self.adapter.getStateAsync(id);
        let value: number | undefined;

        if (state) {
            value = (state.val as number) > 0 ? (state.val as number) : 0;
        }

        if (newVal !== undefined && newVal >= 0) {
            await self.adapter.setState(id, { val: newVal, ack: true });
        }

        return value;
    } catch {
        return 0;
    }
}

/**
 * Get a favorite position
 *
 * @param self
 * @param id
 */
async function getFavoritePosition(self: ShellyClient, id: string): Promise<ioBroker.StateValue | undefined> {
    const node = `${self.getDeviceId()}.${id}`;
    const state = await self.adapter.getStateAsync(node);
    return state ? state.val : undefined;
}

/**
 * Get state
 *
 * @param self
 * @param id
 */
async function getState(self: ShellyClient, id: string): Promise<ioBroker.StateValue | undefined> {
    const node = `${self.getDeviceId()}.${id}`;
    const state = await self.adapter.getStateAsync(node);
    return state ? state.val : undefined;
}

export {
    getIcon,
    celsiusToFahrenheit,
    fahrenheitToCelsius,
    setDeviceName,
    setChannelName,
    getExtTemp,
    getExtHum,
    getLightsObjectColor,
    getLightsObjectWhite,
    intToHex,
    hextoInt,
    getHsvFromRgb,
    getColorsFromHue,
    getColorsFromRGBW,
    getPowerFactor,
    getTotalSumm,
    getTotalReturnedSumm,
    getCurrentSumm,
    getPowerSumm,
    getVoltageCalc,
    getSetDuration,
    getFavoritePosition,
    getRGBW,
    getState,
};
