//
// see specification of BTHome at https://bthome.io/
//
// TODO:
// remove non standard gust_speed
...
/** One BTHome object-id definition: property name, value encoding, optional scale/unit/event flag. */
interface BthEntry {
    /** Property name, e.g. `temperature`. */
    n: string;
    /** Value encoding (one of the `uint*`/`int*`/`rawtext` constants above). */
    t: number;
    /** Optional scale factor applied to the decoded raw integer. */
    f?: number;
    /** Optional unit string. */
    u?: string;
    /** Optional marker for numbered event objects (e.g. button). */
    b?: number;
}

const BTH: Record<number, BthEntry> = {
    // Misc
    0x00: { n: 'pid', t: uint8 },
    ...
    // Binary Sensor data
    0x15: { n: 'battery_low', t: uint8 } /* standard name battery */,
...
class BleDecoder {
    // Reinterprets an unsigned integer as a signed two's-complement value of `bitsz` bits.
    utoi(num: number, bitsz: number): number {
        const mask = 1 << (bitsz - 1);
        return num & mask ? num - (1 << bitsz) : num;
    }

    // The `!` on `buffer.at(i)` is safe: every caller has already verified the buffer is long
    // enough via `getBufValue`/`getByteSize`. (Out of range, `undefined << n` coerces to 0 in JS,
    // so even an unexpected miss is runtime-identical to the original code.)
    getUInt8(buffer: Buffer): number {
        return buffer.at(0)!;
    }
    getInt8(buffer: Buffer): number {
        return this.utoi(this.getUInt8(buffer), 8);
    }
    getUInt16LE(buffer: Buffer): number {
        return 0xffff & ((buffer.at(1)! << 8) | buffer.at(0)!);
    }
    getInt16LE(buffer: Buffer): number {
        return this.utoi(this.getUInt16LE(buffer), 16);
    }
    getUInt24LE(buffer: Buffer): number {
        return 0x00ffffff & ((buffer.at(2)! << 16) | (buffer.at(1)! << 8) | buffer.at(0)!);
    }
    getInt24LE(buffer: Buffer): number {
        return this.utoi(this.getUInt24LE(buffer), 24);
    }
    getUInt32LE(buffer: Buffer): number {
        return 0x00ffffffff & ((buffer.at(3)! << 24) | (buffer.at(2)! << 16) | (buffer.at(1)! << 8) | buffer.at(0)!);
    }
    getInt32LE(buffer: Buffer): number {
        return this.utoi(this.getUInt32LE(buffer), 32);
    }
    getBufValue(type: number, buffer: Buffer): number | null {
    ...
    getByteSize(type: number): number {
    ...
    unpack(buffer: Buffer, encryptedPayload?: boolean): Record<string, unknown> | null {
        ...
        let _value: number | string | null = null;
        let _name = '';
        let _btnNum = 1;

        while (buffer.length > 0) {
            const _byteId = buffer.at(0)!; // save ID
            const _bth = BTH[_byteId];
