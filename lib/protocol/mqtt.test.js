...
describe('Test external MQTT topic handling', function () {
    ...
    it('identifies shelly-folder and bare Gen2 status topic prefixes and ignores other-folder topics', function () {
        const server = new MQTTServerExternal(createAdapter(), {}, {});

        expect(server._identifyDevicePrefix('shelly/shellyplus1-ABC123/status/switch:0')).to.equal(
            'shelly/shellyplus1-ABC123',
        );
        // Bare prefix (Shelly default) must also be recognized
        expect(server._identifyDevicePrefix('shellyplus1-ABC123/status/switch:0')).to.equal('shellyplus1-ABC123');
        // Other folder prefixes must be ignored
        expect(server._identifyDevicePrefix('room/shellyplus1-ABC123/status/switch:0')).to.equal(null);
    });

    it('accepts shelly-folder and plain stored Mqtt.topicPrefix values in external mode', async function () {
        ...
        // Plain bare prefix must be returned as-is (not converted to shelly/)
