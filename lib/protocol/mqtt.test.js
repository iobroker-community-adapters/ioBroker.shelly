...
describe('Test external MQTT topic handling', function () {
    ...
    it('should reject multi-segment stored Mqtt.topicPrefix and use fallback', async function () {
    ...
    it('maps INIT_SRC responses to existing clients with equivalent folder/non-folder prefixes', async function () {
        const handled = [];
        const server = new MQTTServerExternal(createAdapter(), {}, {});
        const knownClient = {
            handleMessage: async (topic, payload) => handled.push({ topic, payload: payload.toString() }),
        };
        server.deviceClients['room/shellyplus1-ABC123'] = knownClient;

        await server._handleInitSrcMessage(
            'iobroker-init/rpc',
            Buffer.from(JSON.stringify({ src: 'shellyplus1-ABC123', method: 'NotifyStatus' })),
        );

        expect(handled).to.deep.equal([
            {
                topic: 'iobroker-init/rpc',
                payload: '{"src":"shellyplus1-ABC123","method":"NotifyStatus"}',
            },
        ]);
        expect(server.deviceClients['shellyplus1-ABC123']).to.equal(undefined);
    });

    it('does not map ambiguous INIT_SRC prefixes to an existing client', function () {
        const server = new MQTTServerExternal(createAdapter(), {}, {});
        const clientA = {};
        const clientB = {};
        server.deviceClients['roomA/shellyplus1-ABC123'] = clientA;
        server.deviceClients['roomB/shellyplus1-ABC123'] = clientB;

        const matched = server._findEquivalentClientByPrefix('shellyplus1-ABC123');

        expect(matched).to.equal(undefined);
    });
});
