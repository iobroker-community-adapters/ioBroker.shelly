...
describe('Test MQTT external folder prefixes', function () {
    ...
    it('updates the client mapping to the observed folder-prefixed topic', function () {
        ...
        server.deviceClients['shellyplus1-abc123'] = fakeClient;
        server._updateClientPrefixMapping('shellyplus1-abc123', 'room/shellyplus1-abc123', fakeClient);

        expect(server.deviceClients['shellyplus1-abc123']).to.equal(undefined);
        expect(server.deviceClients['room/shellyplus1-abc123']).to.equal(fakeClient);
        expect(updatedPrefixes).to.deep.equal(['room/shellyplus1-abc123']);
    });
    ...
    it('falls back to the derived prefix when reading Mqtt.topicPrefix fails', async function () {
        const server = new MQTTServerExternal(
            createAdapter({
                getStateAsync: async () => {
        ...
        expect(prefix).to.equal('shellyplus1-ABC123');
    });

    it('falls back to the derived prefix for empty, whitespace-only, and non-string stored prefixes', async function () {
        const values = [{ val: '' }, { val: '   ' }, { val: 5 }, { val: { prefix: 'room/shellyplus1-ABC123' } }];

        for (const value of values) {
            const server = new MQTTServerExternal(
                createAdapter({
                    getStateAsync: async () => value,
                }),
                {},
                {},
            );
