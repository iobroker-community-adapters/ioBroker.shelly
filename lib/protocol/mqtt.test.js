...
describe('Test external MQTT topic handling', function () {
    ...
    it('identifies only normal Gen2 status topic prefixes for live switch updates and ignores shelly-folder topics', function () {
        const server = new MQTTServerExternal(createAdapter(), {}, {});

        expect(server._identifyDevicePrefix('shellyplus1-ABC123/status/switch:0')).to.equal('shellyplus1-ABC123');
        expect(server._identifyDevicePrefix('room/shellyplus1-ABC123/status/switch:0')).to.equal(null);
        expect(server._identifyDevicePrefix('shelly/shellyplus1-ABC123/status/switch:0')).to.equal(null);
    });

    it('accepts only normal stored Mqtt.topicPrefix values in external mode', async function () {
