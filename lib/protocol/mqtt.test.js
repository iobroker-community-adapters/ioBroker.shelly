...
describe('Test external MQTT topic handling', function () {
    it('ignores topics that do not start with the "shelly/" prefix', function () {
        const server = new MQTTServerExternal(createAdapter(), {}, {});

        // Legacy gen1 prefix must be ignored in external mode
        expect(server._identifyDevicePrefix('shellies/shellyswitch25-ABC123/relay/0')).to.equal(null);
        // Folder-prefixed variants must be ignored
