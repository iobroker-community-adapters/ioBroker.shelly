...
class CoAPClient extends BaseClient {
    constructor(adapter, objectHelper, eventEmitter, shelly, deviceId, ip, payload, description) {
        ...
        this.shelly = shelly;
        this.devicename = deviceId;   // e.g. SHRGBW2#D88040#2
        this.ip = ip;                 // e.g. 192.168.1.2

        this.adapter.log.debug(`[CoAP] Starting new client. Device ID: "${this.devicename}", IP: ${this.ip}`);
    ...
    getDeviceClass() {
        if (!this.deviceClass) {
            this.deviceClass = datapoints.getDeviceNameForCoAP(this.getDeviceType());
        }
        return this.deviceClass;
    }

    /**
     * Extracts serial id from device id
     * Example, you get 12345 for shellyplug-s-12345
     */
    getSerialId() {
        if (!this.serialid) {
            const devicename = this.getDeviceName();
    ...
    async createIoBrokerState(payload) {
        ...
        for (const i in dps) {
            const dp = dps[i];
            const deviceName = this.getDeviceName();
            const stateid = `${deviceName}.${dp.state}`;
            let value = payload;
