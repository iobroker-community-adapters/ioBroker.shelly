...
class CoAPServer extends BaseServer {
    ...
    listen() {
        ...
        shelly.on('update-device-status', (deviceId, status) => {0
            this.adapter.log.debug(`[CoAP Server] Status update received for ${deviceId}: ${JSON.stringify(status)}`);

            if (this.isDestroyed) {
                this.adapter.log.debug(`[CoAP Server] instance is destroyed - ignoreing status update`);
                return;
            }
    ...
    destroy() {
        ...
        for (const deviceId in this.clients) {
            this.adapter.log.debug(`[CoAP Server] destroying client ${deviceId}`);
            this.clients[deviceId].destroy();
            delete this.clients[deviceId];
        }
    }
