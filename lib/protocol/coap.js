...
class CoAPClient extends BaseClient {
    constructor(adapter, objectHelper, eventEmitter, shelly, devicename, ip, payload, description) {
        ...
        this.shelly = shelly;
        this.devicename = devicename; // e.g. SHRGBW2#D88040#2
        this.ip = ip;                 // e.g. 192.168.1.2

        this.adapter.log.debug(`[CoAP] Starting new client. Devicename: "${this.devicename}", IP: ${this.ip}`);
    ...
    async start(payload, description) {
        if (this.deviceExists()) { // needs getDeviceId()
