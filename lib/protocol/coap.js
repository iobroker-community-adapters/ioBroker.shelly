...
class CoAPClient extends BaseClient {
     ...
     * @param {object} payload - object can be every type of value
     */
    async createIoBrokerState(payload) {
        ...
        for (const i in dps) {
            ...
            this.adapter.log.silly(`[CoAP] Message with value for ${this.getLogInfo()}: state: ${fullStateId}, payload: ${JSON.stringify(payload)}`);
            ...
            try {
                ...
                if (value !== undefined) {
                    ...
                    if (value !== undefined && (!Object.prototype.hasOwnProperty.call(this.states, fullStateId) || this.states[fullStateId] !== value || this.adapter.config.updateUnchangedObjects)) {
                        this.adapter.log.debug(`[CoAP] State change ${this.getLogInfo()}: state: ${fullStateId}, value: ${JSON.stringify(value)}`);
                ...
                this.adapter.log.error(`[CoAP] Error ${error} in function dp.coap.coap_publish_funct of state ${fullStateId} for ${this.getLogInfo()}`);
            }
        }
