...
function createStatefulAdapter(config = {}) {
    const adapter = createFakeAdapter(config);
    const objects = {};
    const states = {};
    adapter.setObject = sinon.stub((id, obj, callback) => {
        objects[`shelly.0.${id}`] = { _id: `shelly.0.${id}`, ...obj };
        callback?.();
    });
    adapter.extendObject = sinon.stub((id, obj, callback) => {
        const fullId = id.startsWith('shelly.0.') ? id : `shelly.0.${id}`;
        objects[fullId] = {
            ...(objects[fullId] || { _id: fullId }),
            ...obj,
            common: {
                ...(objects[fullId]?.common || {}),
                ...(obj.common || {}),
            },
        };
        callback?.();
    });
    adapter.getObject = sinon.stub((id, callback) => {
        const fullId = id.startsWith('shelly.0.') ? id : `shelly.0.${id}`;
        callback(null, objects[fullId] || null);
    });
    adapter.getObjectAsync = sinon.stub(async id => {
        const fullId = id.startsWith('shelly.0.') ? id : `shelly.0.${id}`;
        return objects[fullId];
    });
    adapter.getAdapterObjects = sinon.stub(callback => callback(objects));
    adapter.getAdapterObjectsAsync = sinon.stub().resolves(objects);
    adapter.setState = sinon.stub((id, value, ackOrCallback, maybeCallback) => {
        const callback = typeof ackOrCallback === 'function' ? ackOrCallback : maybeCallback;
        const ack = typeof ackOrCallback === 'boolean' ? ackOrCallback : value?.ack;
        states[id] = typeof value === 'object' && value && Object.prototype.hasOwnProperty.call(value, 'val')
            ? value
            : { val: value, ack };
        callback?.();
        return Promise.resolve();
    });
    adapter.getStateAsync = sinon.stub(async id => states[id]);
    adapter.__objects = objects;
    adapter.__states = states;
    return adapter;
}
...
describe('protocol/http helpers', () => {
    ...
    it('maps known Gen1 Relay0 power and energy from /status into ioBroker states', async () => {
        const adapter = createFakeAdapter({ polltime: 0, httpDebugCommands: true });
        const writes = [];
        const objectHelper = {
    ...
    it('logs raw HTTP response bodies only when raw response debug is enabled', async () => {
        const adapter = createFakeAdapter({ polltime: 0, httpSaveRawJson: true, httpDebugRawResponses: false });
        const client = new HTTPPollingClient(
            adapter,
            { setOrUpdateObject: () => {}, processObjectQueue: () => {} },
            { on: () => {} },
        ...
        client.deviceGen = 1;
        client.ip = '192.168.1.10';
        sinon.stub(client, 'requestWithRetry').resolves('{"ok":true}');

        await client.pollRawPayloads();

        assert.equal(adapter.logMessages.some(message => message.includes('[HTTP Raw Debug]')), false);

        adapter.config.httpDebugRawResponses = true;
        await client.pollRawPayloads();

        assert.equal(adapter.logMessages.some(message => message.includes('[HTTP Raw Debug]')), true);
    });
    ...
    it('summarizes discovery scan results instead of logging each failed IP by default', async () => {
        const adapter = createFakeAdapter({
            httpDiscoveryEnabled: true,
            httpMaxParallel: 1,
            httpNetworks: [{ enabled: true, range: '192.168.1.10-12' }],
        });
        const server = new HTTPPollingServer(
            adapter,
            { setOrUpdateObject: () => {}, processObjectQueue: () => {} },
        ...
        sinon.stub(server, 'probeIp').callsFake(async ip =>
            ip.endsWith('.10')
                ? { ip, deviceId: 'SHPLG-S#AABBCC001122#1', source: 'http-discovery', enabled: true }
                : undefined,
        );

        await server.scanNetworkRanges();

        assert(
            adapter.logMessages.some(message =>
                message.includes('[HTTP] Discovery summary: scanned=3, found=1, failed=2'),
            ),
        );
        assert.equal(adapter.logMessages.some(message => message.includes('Discovery probe failed')), false);
    });
    ...
    it('fully initializes registry HTTP clients and registers Relay0.Switch triggers on startup', async () => {
        const requests = [];
        const httpDevice = await createTestHttpServer((req, res) => {
            requests.push(req.url);
            res.setHeader('content-type', 'application/json');
            if (req.url === '/shelly') {
                res.end(JSON.stringify({ type: 'SHPLG-S', mac: 'AABBCC009999' }));
                return;
            }
            if (req.url === '/settings') {
                res.end(JSON.stringify({ name: 'Registry test plug', mode: 'relay' }));
                return;
            }
            if (req.url === '/status') {
                res.end(JSON.stringify({ relays: [{ ison: true }], meters: [{ power: 1, total: 60 }] }));
                return;
            }
            if (req.url === '/relay/0?turn=off') {
                res.end(JSON.stringify({ ison: false }));
                return;
            }
            res.statusCode = 404;
            res.end('{}');
        });
        const deviceId = 'SHPLG-S#AABBCC009999#1';
        const adapter = createStatefulAdapter({
            polltime: 0,
            httpDiscoveryEnabled: false,
            httpDebugCommands: true,
            httpRetries: 0,
        });
        adapter.__objects[`shelly.0.${deviceId}`] = {
            _id: `shelly.0.${deviceId}`,
            type: 'device',
            common: { name: 'Registry test plug' },
            native: {},
        };
        adapter.__states[`${deviceId}.hostname`] = { val: httpDevice.host, ack: true };
        adapter.getDevicesAsync.resolves([adapter.__objects[`shelly.0.${deviceId}`]]);
        realObjectHelper.init(adapter);
        const server = new HTTPPollingServer(adapter, realObjectHelper, { on: () => {} });

        try {
            await server.listen();

            assert(realObjectHelper.getStateChangeTriggerKeys().includes(`${deviceId}.Relay0.Switch`));
            assert(
                adapter.logMessages.some(
                    message =>
                        message.includes('[HTTP CMD] registering stateChangeTrigger') &&
                        message.includes(`${deviceId}.Relay0.Switch`),
                ),
            );

            realObjectHelper.handleStateChange(`shelly.0.${deviceId}.Relay0.Switch`, {
                val: false,
                ack: false,
                from: 'system.adapter.test.0',
            });
            ...
            assert(requests.includes('/relay/0?turn=off'));
        } finally {
            server.destroy();
            await httpDevice.close();
        }
    });
