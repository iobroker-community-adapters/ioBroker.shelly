...
describe('protocol/http helpers', () => {
    ...
    it('persists discovered HTTP devices without credentials', async () => {
        const adapter = createFakeAdapter({
            httpDiscoveredDevices: [],
            httpDefaultPassword: 'globalValueToMask',
            httpDevices: [
                {
                    ip: '192.168.1.50',
        ...
        await server.persistDiscoveredDevices([
            {
                ip: '192.168.1.10',
                deviceId: 'SHPLG-S#AABBCC001122#1',
                id: 'SHPLG-S#AABBCC001122#1',
        ...
        assert.equal(adapter.config.httpDiscoveredDevices.length, 1);
        assert.equal(adapter.config.httpDiscoveredDevices[0].deviceId, 'SHPLG-S#AABBCC001122#1');
        assert.equal(adapter.config.httpDiscoveredDevices[0].password, undefined);
        assert.equal(adapter.config.httpDiscoveredDevices[0].username, undefined);
        assert.equal(adapter.extendForeignObjectAsync.calledOnce, true);
        const persistedNative = adapter.extendForeignObjectAsync.firstCall.args[1].native;
        assert.equal(persistedNative.httpDiscoveredDevices[0].password, undefined);
        assert.equal(persistedNative.httpDiscoveredDevices[0].username, undefined);
    });

    it('starts persisted HTTP devices without running discovery when discovery is disabled', async () => {
        const adapter = createFakeAdapter({
        ...
        assert(adapter.logMessages.some(message => message.includes('Skipping network discovery because discovery=false')));
    });

    it('merges manual and persisted HTTP devices with manual configuration taking precedence', async () => {
        const adapter = createFakeAdapter({
            httpDiscoveryEnabled: false,
            httpDevices: [
                {
    ...
    it('registers Relay0.Switch trigger after restart from persisted HTTP device without discovery', async () => {
