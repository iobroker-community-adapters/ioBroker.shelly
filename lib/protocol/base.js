...
class BaseClient {
    ...
    */
    async httpIoBrokerState() {
        ...
        for (const httpUri in this.http) {
            const states = this.http[httpUri];
            try {
                const body = await this.requestAsync('http://' + this.getIP() + httpUri);
                    ...
                    if (state && state.state) {
                        ...
                        try {
                            ...
                            if (error.name && error.name.startsWith('TypeError')) {
                                this.adapter.log.debug(`[httpIoBrokerState] Could not find property for state ${stateid} for ${this.getName()}: ${error}`);
                            } else {
                                if (polltime > 0 && polltime < 60) polltime = 60;
                                if (body && body === '401 Unauthorized') {
                                    this.adapter.log.error(`[httpIoBrokerState] Error for state ${stateid} for ${this.getName()}: Wrong http username or http password! Please enter user credentials for restricted login.`);
                                    break;
                                } else {
                                    this.adapter.log.error(`[httpIoBrokerState] Error for state ${stateid} for ${this.getName()}: ${error} - value: "${value}"`);
                                }
                            }
                        }
                    }
                ...
                this.adapter.log.debug(`[httpIoBrokerState] HTTP request error: ${JSON.stringify(error)}`);
            }
        }
