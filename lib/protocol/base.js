...
class BaseClient {
    ...
    async httpIoBrokerState() {
        ...
        for (const httpUrl in this.http) {
            if (this.isDestroyed) {
                this.adapter.log.silly('[httpIoBrokerState] destroy in progress - cancel processing');
                return; // abort if shutting down
            }
        ...
        if (!this.isDestroyed && this.http && Object.keys(this.http).length > 0 && polltime > 0) {
