...
class BaseClient {
    ...
    */
        ...
        return new Promise((resolve, reject) => {
            try {
                if (Object.keys(this.device).length === 0) {
                    ...
                    let deviceStates = datapoints.getDeviceByType(this.getDeviceClass(), this.type, this.getDeviceMode());
                    if (deviceStates) {
                        if (this.type === 'coap') {
                        ...
                        for (const statename in deviceStates) {
                            ...
                            if (state?.[this.type]?.http_publish && !hasTypePublishCmd) {
                                ...
                                if (!deviceStatesHttp[key]) {
                                    deviceStatesHttp[key] = [statename];
                                } else {
                                    deviceStatesHttp[key].push(statename);
                                }
                            }
    ...
    async deleteOldStates() {
        ...
        let dps = datapoints.getAll(this.type, this.getDeviceMode());
        dps = dps[deviceClass];
