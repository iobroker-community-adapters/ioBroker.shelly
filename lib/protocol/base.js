...
class BaseClient {
    ...
    */
     ...
     */
    getLogInfo() {
        return `${this.getIP() ?? '<unknown IP>'} (${this.getDeviceClass()} / ${this.getId()}  / ${this.getDeviceId()})`;
    }
    ...
    async httpIoBrokerState() {
        if (!this.isOnline() || !this.getIP()) {
            this.adapter.log.silly(`[httpIoBrokerState] Device ${this.getLogInfo()} is offline (or IP is unkown) - waiting`);
        ...
        for (const httpUrl in this.http) {
            ...
            try {
                ...
                for (const s in states) {
                    const state = this.device[states[s]];
                    if (state && state.state) {
                        const fullStateId = `${this.getDeviceId()}.${state.state}`;
                        let value = body;
                        try {
        ...
        return new Promise((resolve, reject) => {
            try {
                if (Object.keys(this.device).length === 0) {
                    ...
                    let deviceStates = datapoints.getDeviceByClass(this.getDeviceClass(), this.type, this.getDeviceMode());
                    if (deviceStates) {
                        if (this.type === 'coap') {
                                ...
                                controlFunction = async (value) => {
                                    ...
                                    if (state[this.type] && state[this.type].http_cmd_funct) {
                                        try {
                                            ...
                                            this.adapter.log.debug(`[http controlFunction] Executing state.${this.type}.http_cmd_funct of state ${fullStateId} for ${this.getLogInfo()} with value: ${JSON.stringify(value)}`);
                                        } catch (error) {
                                            this.adapter.log.error(`[http controlFunction] Error in function state.${this.type}.http_cmd_funct of state ${fullStateId} for ${this.getLogInfo()}: ${error}`);
                                        }
                                    }
                                    ...
                                    if (value == this.states?.[fullStateId]) {
                                        ...
                                        delete this.states[fullStateId];
                                    }
                                };
                            }
                                ...
                                controlFunction = async (value) => {
                                    ...
                                    if (state[this.type] && state[this.type].mqtt_cmd_funct) {
                                        try {
                                            ...
                                            this.adapter.log.debug(`[mqtt controlFunction] Executing state.${this.type}.mqtt_cmd_funct of state ${fullStateId} for ${this.getLogInfo()} with value: ${value}`);
                                        } catch (error) {
                                            this.adapter.log.error(`[mqtt controlFunction] Error in function state.${this.type}.mqtt_cmd_funct of state ${fullStateId} for ${this.getLogInfo()}: ${error}`);
                                        }
                                    }
        ...
        if (dps) {
            for (const o in objList) {
                ...
                if (tmpObj && tmpObj._id && tmpObj.type) {
                    const stateId = tmpObj._id.replace(id + '.', '');

                    // Just delete states of this device!
                    if (tmpObj.type === 'state' && tmpObj._id.startsWith(id)) {
                        if (!dps[stateId]) {
                            try {
                                ...
                                this.adapter.log.info(`Deleted unused state: ${tmpObj._id}`);
                            } catch (error) {
            ...
            if (tmpObj && tmpObj.type && tmpObj._id && tmpObj.type === 'channel') {
                ...
                if (found === false) {
                    try {
                        ...
                        this.adapter.log.info(`Deleted unused channel: ${tmpObj._id}`);
                    } catch (error) {
