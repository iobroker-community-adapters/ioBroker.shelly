...
class BaseClient {
    constructor(type, adapter, objectHelper, eventEmitter) {
        ...
        this.httpEndpoint; // e.g. 192.168.1.2 or 192.168.1.3:23456 (range extender NAT)
    ...
    async setHttpEndpoint(endpoint, source) {
        if (!endpoint) {
            return;
        }
     ...
     */
    getLogInfo() {
        const httpEndpoint = this.httpEndpoint && this.httpEndpoint !== this.ip ? ` via ${this.httpEndpoint}` : '';
        return `${this.getIP() ?? ''}${httpEndpoint} (${this.getDeviceClass()} / ${this.getId()} / ${this.getDeviceId()})`.trim();
    }
