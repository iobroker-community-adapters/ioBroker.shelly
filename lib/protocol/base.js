                        ...
                        try {
                            ...
                            if (error.name && error.name.startsWith('TypeError')) {
                                ...
                                if (body && body === '401 Unauthorized') {
                                    this.adapter.log.error(`[httpIoBrokerState] Error in function httpIoBrokerState for state ${stateid} for ${this.getName()}: Wrong http username or http password! Please enter user credentials for restricted login.`);
                                    break;
                                } else {
                                    this.adapter.log.error(`[httpIoBrokerState] Error in function httpIoBrokerState for state ${stateid} for ${this.getName()}: ${error} - value: "${value}"`);
                                }
                            }
                        }
                    }
