    ...
    */
    async httpIoBrokerState() {
        ...
        for (let i in this.http) {
            ...
            try {
                ...
                // this.adapter.log.debug(`HTTP response ${JSON.stringify(body)} for ${this.getName()}`);
                for (let j in states) {
                ...
                this.adapter.log.error(`HTTP request error: ${JSON.stringify(error)}`);
            }
        }
    ...
    createObjects() {
        if (Object.keys(this.device).length === 0) {
            ...
            let devices = datapoints.getDeviceByType(devicetype, this.type);
            if (devices) {
                ...
                for (let j in devices) {
                    ...
                    let controlFunction;
                    if (state[this.type] && state[this.type].http_cmd && !state[this.type].coap_cmd) {
                        controlFunction = async (value) => {
                            ...
                            try {
                                if (this.auth) {
                                    params = {
                                        url: 'http://' + this.getIP() + state[this.type].http_cmd,
                                        timeout: this.httptimeout,
                                        qs: value,
                                        headers: {
                                            'Authorization': this.auth
                                ...
                                if (body && body === '401 Unauthorized') {
                                    ...
                                    this.adapter.log.error(`Error in function state.${this.type}.http_cmd for state ${stateid} and request ${JSON.stringify(params)} for ${this.getName()}: ${error}`);
                                }
                            }
                            delete this.states[stateid];
                        };
                    }
                    if (state[this.type] && state[this.type].http_publish && !state[this.type].coap_publish) {
                    ...
                    let value;
                    if (state[this.type].coap_init_value) value = state[this.type].coap_init_value;
                    this.objectHelper.setOrUpdateObject(stateid, {
                        type: 'state',
                        common: state.common
