    ...
    async executeHttpControlCommand(httpState, publishValue, fullStateId) {
        if (this.getDeviceGen() === 1) {
            ...
            this.adapter.log.debug(
                `[http controlFunction] Executing gen 1 state.${this.type}.http_cmd of state ${fullStateId} for ${this.getLogInfo()} from url: ${url.pathname}${url.search}`,
            );
        ...
        if (this.getDeviceGen() >= 2 && httpState?.mqtt_cmd && this.publishStateValue !== undefined) {
            this.adapter.log.debug(
                `[http controlFunction] Executing Gen ${this.getDeviceGen()} RPC command of state ${fullStateId} for ${this.getLogInfo()}`,
            );
                        ...
                        if (this.shouldRegisterHttpControlFunction(state, stateId, hasTypeCmd, httpState)) {
                            ...
                            controlFunction = async value => {
                                if (this.isOnline()) {
                                    ...
                                    this.adapter.log.silly(
                                        `[http controlFunction] Entered HTTP command of state ${fullStateId} for ${this.getLogInfo()} with value: ${value}`,
                                    );
                                    ...
                                    if (commandSuccessful) {
                                        this.adapter.log.debug(
                                            `[http controlFunction] Ack HTTP command state ${fullStateId} for ${this.getLogInfo()} and refresh polling`,
                                        );
