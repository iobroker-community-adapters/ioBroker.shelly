...
class BaseClient {
    ...
    async addVirtualComponentState(state){
        Object.assign(this.virtualComponentStates, state);
    }
    ...
    async createObjects() {
        return new Promise((resolve, reject) => {
            try {
                ...
                if (this.virtualComponentStates && Object.keys(this.virtualComponentStates).length != 0){
                    this.adapter.log.debug(
                        `[createObjects] adding additional states for ${this.getLogInfo()} ${JSON.stringify(this.virtualComponentStates)}'}`,
                    );
                    Object.assign(deviceStates, this.virtualComponentStates);
                }
