...
class BaseClient {
    ...
    async requestAsync(url) {
        ...
        const httpDebugDir = `httpDebug/${this.getId().replace(/[^a-zA-Z0-9-_.]/g, '_')}`;
    ...
    */
     ...
     */
    getDeviceId() {
        throw new Error('You have to implement the method getDeviceId!');
    }
    ...
    sendState2Client(cmd, value, qos) {
        throw new Error('You have to implement the method sendState2Client!');
    }
                            ...
                            if (state[this.type] && state[this.type].http_cmd && !hasTypeCmd) {
                                ...
                                controlFunction = async (value) => {
                                    ...
                                    if (value !== this.states?.[stateid]) {
                                        ...
                                        this.adapter.log.debug(`[http controlFunction] Skipped state.${this.type}.http_cmd of state ${stateid} for ${this.getName()}: Value not changed`);
                            ...
                            if (this.type === 'mqtt' && state[this.type] && hasTypeCmd) {
                                ...
                                controlFunction = async (value) => {
                                    ...
                                    if (value !== this.states?.[stateid]) {
                                        ...
                                        this.adapter.log.debug(`[mqtt controlFunction] Skipped state.${this.type}.mqtt_cmd of state ${stateid} for ${this.getName()}: Value not changed`);
                                        await this.adapter.setStateAsync(stateid, { val: value, ack: true });
                                    }
                                };
                            }
    ...
    async downloadAllScripts() {
        ...
        try {
            if (this.getDeviceGen() === 2) {
                const scriptDir = `scripts/${this.getId().replace(/[^a-zA-Z0-9-_.]/g, '_')}`;
                ...
                for (const script of scriptList.scripts) {
                    if (script.id && script.name) {
                        ...
                        await this.adapter.writeFileAsync(this.adapter.namespace, `${scriptDir}/${script.name}.js`, scriptCode.data);
                        downloadCounter++;
                    }
                }
