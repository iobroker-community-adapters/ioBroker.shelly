...
class BaseClient {
    ...
    /**
     * Validates if the serial ID part of device ID contains a valid MAC address format
     * Valid MAC addresses should be hexadecimal (0-9, A-F, a-f) and typically 6 or 12 bytes
     *
     * @param serialId - The serial ID extracted from device ID (e.g., "8CAAB5616291" or "regentonne")
     * @returns true if valid MAC address format, false otherwise
     */
    isValidMacAddress(serialId) {
        if (!serialId || typeof serialId !== 'string') {
            return false;
        }
        // MAC address should be hexadecimal only
        // Common lengths: 6 bytes (12 chars), 12 bytes (24 chars), or partial like 6 chars
        return /^[0-9A-Fa-f]{6,}$/.test(serialId);
    }

    /**
     * Validates the device ID and logs a warning if the serial ID is not in valid MAC address format
     */
    validateDeviceId() {
        const serialId = this.getSerialId();
        const deviceId = this.getDeviceId();

        if (serialId && deviceId && !this.isValidMacAddress(serialId)) {
            this.adapter.log.warn(
                `Device ID "${deviceId}" contains an invalid MAC address format in serial part "${serialId}". ` +
