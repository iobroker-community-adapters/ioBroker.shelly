...
function truncateDebugString(value, maxLength = 500) {
    const text = sanitizeDebugString(value);
...
function collectJsonPaths(value, prefix = '') {
    if (Array.isArray(value)) {
        return value.flatMap((item, index) => collectJsonPaths(item, `${prefix}[${index}]`));
    }
    if (!value || typeof value !== 'object') {
    ...
    for (const [key, item] of Object.entries(value)) {
        ...
        const childPaths = collectJsonPaths(item, path);
...
class BaseClient {
    ...
    isHttpPollingDebugEnabled() {
        const configuredLevel = this.adapter?.log?.level || this.adapter?.log?._level || this.adapter?.logLevel;
        return (
            this.adapter?.config?.logDebugMessages === true ||
            configuredLevel === 'debug' ||
            configuredLevel === 'silly'
        );
    }

    formatHttpDebugBody(body) {
        const parsed = parseDebugJson(body);
        if (parsed !== undefined) {
            return JSON.stringify(redactSensitiveData(parsed), null, 2);
        }
        return sanitizeDebugString(body);
    }

    getHttpDebugPaths(body) {
        const parsed = parseDebugJson(body);
        return parsed ? collectJsonPaths(parsed) : [];
    }

    logHttpPollingDebug(lines) {
        if (!this.isHttpPollingDebugEnabled()) {
            return;
        }
        this.adapter.log.debug(lines.join('\n'));
    }

    logHttpPollingResponse(url, status, durationMs, body) {
        this.logHttpPollingDebug([
            `[HTTP Polling Debug] Polling Device:`,
            `${this.customName || this.getDeviceId() || this.getLogInfo()}`,
            ``,
            `HTTP:`,
            `GET ${url}`,
            ``,
            ...
            ``,
            `Response:`,
            this.formatHttpDebugBody(body),
        ]);
    }

    logHttpPollingExtraction(url, extracted, missing, writes) {
        const lines = [
            `[HTTP Polling Debug] Extraction for ${this.customName || this.getDeviceId() || this.getLogInfo()}`,
            `Source: ${url}`,
            ``,
            `Extracted:`,
            ...(extracted.length
                ? extracted.map(item => `${item.stateId} = ${JSON.stringify(item.value)}`)
                : ['<none>']),
        ];
        ...
        if (missing.length) {
            lines.push(
                ...
                ...missing.map(
                    item =>
                        `${item.stateId} path not found. Available JSON paths: ${item.paths.join(', ') || '<none>'}`,
                ),
            );
        }

        lines.push(``, `Write State:`, ...(writes.length ? writes.map(item => `${item.stateId}`) : ['<none>']));

        this.logHttpPollingDebug(lines);
    }

    logHttpCommandDebug(message, details = {}) {
        const safeDetails = Object.entries(details)
            .filter(([, value]) => value !== undefined)
            .map(([key, value]) => `${key}=${truncateDebugString(JSON.stringify(value))}`)
            .join(', ');
        this.adapter.log.debug(`[HTTP Command Debug] ${message}${safeDetails ? `: ${safeDetails}` : ''}`);
    }

    getHttpProtocolState(state, _stateId) {
        return state?.[this.type];
    }

    shouldCreateHttpPollingState(_state, _stateId, hasTypePublishCmd) {
        return !hasTypePublishCmd;
    }

    hasHttpControlCommand(httpState) {
        return !!httpState?.http_cmd;
    }

    shouldRegisterHttpControlFunction(_state, _stateId, hasTypeCmd, httpState) {
        return this.hasHttpControlCommand(httpState) && !hasTypeCmd;
    }

    shouldRegisterMqttControlFunction(state, hasTypeCmd) {
        return this.type === 'mqtt' && state[this.type] && hasTypeCmd;
    }

    async executeHttpControlCommand(httpState, publishValue, fullStateId) {
        if (this.getDeviceGen() === 1) {
            // Append value parameters
            const url = new URL(`http://${this.getIP()}${httpState.http_cmd}`);
            if (typeof publishValue === 'object') {
                Object.keys(publishValue).forEach(key => {
                    if (publishValue[key] !== undefined && publishValue[key] !== null) {
                        url.searchParams.append(key, publishValue[key]);
                    }
                });
            }

            this.logHttpCommandDebug('selected Gen1 HTTP command route', {
                stateId: fullStateId,
                commandType: 'http-get',
                method: 'GET',
                url: `${url.pathname}${url.search}`,
            });
            const body = await this.requestAsync(url.pathname + url.search);
        ...
        if (this.getDeviceGen() >= 2 && httpState?.mqtt_cmd && this.publishStateValue !== undefined) {
            this.logHttpCommandDebug('selected Gen2+ RPC command route', {
                stateId: fullStateId,
                commandType: 'rpc',
                rpcBody: publishValue,
            });
        ...
        this.adapter.log.error(
            `[http controlFunction] Error in function state.${this.type}.http_cmd of state ${fullStateId} for ${this.getLogInfo()}: Not supported for Gen ${this.getDeviceGen()} devices`,
        );
        return undefined;
    }

    async afterHttpControlCommand(_fullStateId) {
        // Optional hook for protocol implementations.
    }
    ...
    getPollTime() {
        ...
        if (deviceClass) {
            ...
            if (polltime === undefined) {
                polltime = this.adapter.config.polltime || 0;
            }
            if (this.adapter.config.polltime > polltime) {
                polltime = this.adapter.config.polltime;
            }
            return polltime;
        }
    ...
    async httpIoBrokerState() {
        if (!this.isOnline() || !this.getIP()) {
            ...
            this.httpIoBrokerStateTimeout = setTimeout(
                ...
                this.adapter.config.polltime * 1000,
            );
            return;
        }
        ...
        for (const httpUrl in this.http) {
            ...
            try {
                ...
                const debugMissing = [];
                const debugWrites = [];
                const debugPaths = this.isHttpPollingDebugEnabled() ? this.getHttpDebugPaths(body) : [];
                for (const i in dps) {
                    const dp = this.device[dps[i]];
                    if (dp && dp.state) {
                        const httpState = this.getHttpProtocolState(dp, dp.state);
                        ...
                        try {
                            ...
                            if (httpState?.http_publish_funct) {
                                value = isAsync(httpState.http_publish_funct)
                                    ? await httpState.http_publish_funct(value, this)
                                    : httpState.http_publish_funct(value, this);
                            }
                ...
                this.logHttpPollingExtraction(httpUrl, debugExtracted, debugMissing, debugWrites);
            } catch (err) {
            ...
            try {
                ...
                if (deviceStates) {
                    for (const stateId in deviceStates) {
                        ...
                        let controlFunction;
                        const httpState = this.getHttpProtocolState(state, stateId);
                        const shouldRegisterHttpCallback = this.shouldRegisterHttpControlFunction(
                            state,
                            stateId,
                            hasTypeCmd,
                            httpState,
                        );
                        const activeProtocol = this.adapter.config?.protocol || this.type;

                        this.adapter.log.debug(
                            `[createObjects] device=${deviceId}, state=${stateId}, write=${state.common?.write}, role=${state.common?.role}, activeProtocol=${activeProtocol}, profileProtocol=${this.type}, http_cmd=${!!httpState?.http_cmd}, coap_cmd=${!!state?.coap?.coap_cmd}, mqtt_cmd=${!!state?.mqtt?.mqtt_cmd || !!httpState?.mqtt_cmd}, stateChangeCallback=${shouldRegisterHttpCallback}`,
                        );

                        // HTTP
                        if (shouldRegisterHttpCallback) {
                            this.adapter.log.silly(
                                `[http controlFunction] Found HTTP command of state ${fullStateId} for ${this.getLogInfo()}`,
                            );
                            controlFunction = async value => {
                                if (this.isOnline()) {
                                    ...
                                    this.logHttpCommandDebug('executing HTTP command callback', {
                                        adapterMode: this.type,
                                        deviceType: this.getDeviceType(),
                                        deviceClass: this.getDeviceClass(),
                                        generation: this.getDeviceGen(),
                                        stateId: fullStateId,
                                        stateType: state.common?.type,
                                        targetValue: value,
                                        hasHttpCommand: !!httpState?.http_cmd,
                                        hasRpcCommand: !!httpState?.mqtt_cmd,
                                    });

                                    // Modify value with http_cmd_funct (if exists)
                                    if (httpState?.http_cmd_funct) {
                                        try {
                                            publishValue = isAsync(httpState.http_cmd_funct)
                                                ? await httpState.http_cmd_funct(value, this)
                                                : httpState.http_cmd_funct(value, this);
                                            this.adapter.log.debug(
                                                `[http controlFunction] Executing HTTP command function of state ${fullStateId} for ${this.getLogInfo()} with value: ${value} -> ${JSON.stringify(publishValue)}`,
                                            );
                                        } catch (err) {
                                            this.adapter.log.error(
                                                `[http controlFunction] Error in HTTP command function of state ${fullStateId} for ${this.getLogInfo()}: ${err}`,
                                            );
                                        }
                                    }

                                    try {
                                        await this.executeHttpControlCommand(httpState, publishValue, fullStateId);
                                        commandSuccessful = true;
                                    } catch (err) {
                                        if (err && err.response && err.response.status == 401) {
                                            this.adapter.log.error(
                                                `[http controlFunction] Error in HTTP command of state ${fullStateId} for ${this.getLogInfo()}: Wrong http username or http password! Please enter user credentials for restricted login.`,
                                            );
                                        } else {
                                            this.adapter.log.error(
                                                `[http controlFunction] Error in HTTP command of state ${fullStateId} for ${this.getLogInfo()}: ${err}`,
                                            );
                                        }
                                    }

                                    if (commandSuccessful) {
                                        this.logHttpCommandDebug('setting ack=true after HTTP command', {
                                            stateId: fullStateId,
                                            value,
                                        });
                                        ...
                                        this.logHttpCommandDebug('triggering refresh after HTTP command', {
                                            stateId: fullStateId,
                                        });
                                        await this.afterHttpControlCommand(fullStateId);
                                    } else if (value == this.stateValueCache?.[fullStateId]) {
                                        this.adapter.log.debug(
                                            `[http controlFunction] Ack HTTP command state ${fullStateId} for ${this.getLogInfo()} in adapter: Value not changed`,
                                        );
                                        await this.adapter.setState(fullStateId, {
                                            val: value,
                                            ack: true,
                        ...
                        // MQTT
                        if (this.shouldRegisterMqttControlFunction(state, hasTypeCmd)) {
                            this.adapter.log.silly(
                            ...
                            controlFunction = async value => {
                                if (this.isOnline()) {
                                    ...
                                    if (state[this.type]?.mqtt_cmd_funct) {
                                        try {
                                            publishValue = isAsync(state[this.type].mqtt_cmd_funct)
                                                ? await state[this.type].mqtt_cmd_funct(value, this)
                                                : state[this.type].mqtt_cmd_funct(value, this);
                                            this.adapter.log.debug(
                        ...
                        if (
                            httpState?.http_publish &&
                            this.shouldCreateHttpPollingState(state, stateId, hasTypePublishCmd)
                        ) {
                            const key = httpState.http_publish;
                        ...
                        this.adapter.log.debug(
                            `[createObjects] setOrUpdateObject called for ${fullStateId} with stateChangeCallback=${!!controlFunction}`,
                        );
                    }
                }
