                                ...
                                if (this.isOnline()) {
                                    ...
                                    try {
                                        if (this.getDeviceGen() === 1) {
                                            ...
                                            // Not supported
                                            this.adapter.log.error(
                                                `[http controlFunction] Error in function state.${this.type}.http_cmd of state ${fullStateId} for ${this.getLogInfo()}: Not supported for Gen 2+ devices`,
                                            );
                                        }
                                    } catch (err) {
                                        if (err && err.response && err.response.status == 401) {
                                    ...
                                    if (value == this.stateValueCache?.[fullStateId]) {
                                        ...
                                        await this.adapter.setStateAsync(fullStateId, {
                                            val: value,
                                            ack: true,
