...
class BaseClient {
    constructor(type, adapter, objectHelper, eventEmitter) {
        ...
        this.httpTimeoutFirmwareUpdate = 30 * 1000;
    ...
    async requestAsync(url, overrideTimeout) {
        ...
        return new Promise((resolve, reject) => {
            ...
            let axiosRequestObj = {
                ...
                timeout: overrideTimeout ?? this.httpTimeout,
                url: url,
            };
    ...
    */
    ...
    async firmwareUpdate() {
        ...
        if (!this.getIP()) {
            this.adapter.log.debug(`[firmwareUpdate] Device IP not set - skipping ${this.getLogInfo()}`);
            return;
        }
        ...
        try {
            ...
            if (firmwareUpdateState && firmwareUpdateState.val) {
                ...
                if (this.getDeviceGen() === 1) {
                    const body = await this.requestAsync('/ota?update=true', this.httpTimeoutFirmwareUpdate);
                    this.adapter.log.debug(`[firmwareUpdate] Update result for ${this.getLogInfo()}: ${body}`);
                } else if (this.getDeviceGen() === 2) {
                    const body = await this.requestAsync('/rpc/Shelly.Update', this.httpTimeoutFirmwareUpdate);
                    this.adapter.log.debug(`[firmwareUpdate] Update result for ${this.getLogInfo()}: ${body}`);
                }
            }
        } catch (err) {
