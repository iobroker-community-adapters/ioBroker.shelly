        ...
        return new Promise((resolve, reject) => {
            ...
            if (this.adapter.config.httppassword) {
                if (this.getDeviceGen() === 1) {
                    ...
                    .then(response => {
                        this.adapter.log.silly(`[requestAsync] HTTP response of gen 1 device with basic auth: "${axiosRequestObj.baseURL}${url}" -> "${response.data}"`);
                ...
                this.adapter.log.silly(`[requestAsync] HTTP request to gen ${this.getDeviceGen()} device without auth: "${axiosRequestObj.baseURL}${url}"`);

                axios(
                    axiosRequestObj
                ).then(response => {
                    this.adapter.log.silly(`[requestAsync] HTTP response of gen ${this.getDeviceGen()} device without auth: "${axiosRequestObj.baseURL}${url}" -> "${response.data}"`);
                    resolve(response.data);
                })
                .catch(reject);
            }
        ...
        for (const httpUrl in this.http) {
            ...
            try {
                ...
                if (err && err.response && err.response.status == 401) {
                    this.adapter.log.error(`[httpIoBrokerState] HTTP request error for ${this.getName()}: Wrong http username or http password! Please enter user credentials for restricted login.`);
                } else {
                    this.adapter.log.debug(`[httpIoBrokerState] HTTP request error for ${this.getName()}: ${err}`);
                }
            }
        }
                ...
                if (Object.keys(this.device).length === 0) {
                    ...
                    if (deviceStates) {
                        ...
                        for (const statename in deviceStates) {
                            ...
                            // Fill http object for httpIoBrokerState()
