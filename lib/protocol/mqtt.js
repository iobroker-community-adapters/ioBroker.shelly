...
class MQTTServerExternal extends BaseServer {
    ...
    listen() {
        ...
        this.mqttClientConnection.on('connect', async () => {
            ...
            const topics = ['shellies/#', '+/events/#', '+/rpc', '+/online', '+/debug/#', `${INIT_SRC}/#`];
    ...
    async _handleInitSrcMessage(topic, payload) {
        try {
            ...
            if (!client) {
                client = this._findEquivalentClientByPrefix(srcDevice);
            }
            if (!client) {
    ...
    /**
     * Find an already initialized client when the broker reports an equivalent
     * topic prefix variant (e.g. "id" vs "folder/id").
     *
     * @param {string} srcDevice - Prefix from INIT_SRC payload
     * @returns {ExternalMQTTDeviceClient|undefined}
     */
    _findEquivalentClientByPrefix(srcDevice) {
        if (!srcDevice) {
            return undefined;
        }

        const candidatePrefixes = Object.keys(this.deviceClients).filter(
            prefix => prefix.endsWith(`/${srcDevice}`) || srcDevice.endsWith(`/${prefix}`),
        );

        if (candidatePrefixes.length === 1) {
            const matchedPrefix = candidatePrefixes[0];
            this.adapter.log.debug(
                `[MQTT External] INIT_SRC prefix "${srcDevice}" mapped to known client "${matchedPrefix}"`,
            );
            return this.deviceClients[matchedPrefix];
        }

        if (candidatePrefixes.length > 1) {
            this.adapter.log.warn(
                `[MQTT External] INIT_SRC prefix "${srcDevice}" is ambiguous (${candidatePrefixes.join(', ')})`,
            );
        }
    ...
    async _getStoredDevicePrefix(deviceId, fallbackPrefix) {
        try {
            ...
            if (this._isSingleSegmentPrefix(storedPrefixValue)) {
