...
class MQTTClient extends BaseClient {
    ...
    async processPublishPacket(packet) {
        ...
        try {
            ...
            // Log error and continue processing remaining packets in the queue
            // This ensures one malformed packet doesn't block processing of subsequent packets
            this.adapter.log.error(
                `[MQTT] Error processing publish packet for ${this.getLogInfo()} - topic: ${packet?.topic || 'unknown'}, error: ${err?.message || err}`,
            );
        }
    }
