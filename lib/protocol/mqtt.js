                        ...
                        if (this.getDeviceGen() == 2) {
                            try {
                                ...
                                this.client.publish({
                                    ...
                                    payload: JSON.stringify({id: 0, src: 'iobroker-init', method: 'Shelly.GetDeviceInfo', params: { ident: true }}),
                                ...
                                this.client.publish({
                                    topic: `${this.mqttprefix}/rpc`,
                                    payload: JSON.stringify({id: 0, src: 'iobroker-init', method: 'Shelly.GetStatus' }),
                                    qos: this.adapter.config.qos,
                                    messageId: this.messageId,
                                });
                            } catch (err) {
        ...
        this.client.on('publish', async (packet) => {
            ...
            if (packet.payload) {
                ...
                if (this.getDeviceGen() == 2) {
                    ...
                    if (packet.topic == 'iobroker-init/rpc') {
                        try {
                            const payloadObj = JSON.parse(packet.payload.toString());
