...
class MQTTClient extends BaseClient {
    ...
    getId() {
        if (!this.id) {
            if (this.packet && this.packet.clientId) {
                this.id = this.packet.clientId;
            }
        }
        return this.id;
    }
        ...
        for (const i in dps) {
            ...
            try {
                if (this.replacePrefixIn(dp.mqtt.mqtt_publish) === topic) {
                    ...
                    if (value !== undefined && (!Object.prototype.hasOwnProperty.call(this.states, fullStateId) || this.states[fullStateId] !== value || this.adapter.config.updateUnchangedObjects)) {
                        this.adapter.log.debug(`[MQTT] State change ${this.getLogInfo()}: ${topic} -> state: ${fullStateId}, value: ${JSON.stringify(value)}`);
                        this.states[fullStateId] = value;
    ...
    async finishDeviceOnboarding() {
        if (!this.getDeviceId()) {
            this.adapter.log.error(`[finishDeviceOnboarding] Unable to create states - getDeviceId() returned empty result`);
            return;
        }

        if (!this.getDeviceClass()) {
            this.adapter.log.error(`[finishDeviceOnboarding] Unable to create states - getDeviceClass() returned empty result`);
            return;
        }

        if (!this.getIP()) {
            this.adapter.log.error(`[finishDeviceOnboarding] Unable to create states - getIP() returned empty result`);
            return;
        }
        ...
        this.adapter.log.info(`[finishDeviceOnboarding] Onboarding of device ${this.getLogInfo()} completed`);
    }
        ...
        this.client.on('connect', async (packet) => {
            ...
            if (this.deviceExists()) {
                if (packet.username === this.adapter.config.mqttusername && packet.password !== undefined && packet.password.toString() === this.adapter.config.mqttpassword) {
                    ...
                    // Create objects for type
                    await this.createObjects();

                    // Device onboarding check
                    const onboardingTimeSec = 10;
                    setTimeout(() => {
                        if (!this.onboardingCompleted) {
                            this.adapter.log.error(`[MQTT] Onboarding of device with client id "${packet.clientId}" was not completed within ${onboardingTimeSec} seconds. Please check device configuration.`);
                        }
                    }, onboardingTimeSec * 1000);
                    ...
                    try {
                        ...
                        if (this.getDeviceGen() == 2) {
                            try {
                                ...
                                this.client.publish({
                                    topic: `${this.getMqttPrefix()}/rpc`,
                                    payload: JSON.stringify({id: 0, src: INIT_SRC, method: 'Shelly.GetDeviceInfo', params: { ident: true }}),
                                ...
                                this.client.publish({
                                    topic: `${this.getMqttPrefix()}/rpc`,
                                    payload: JSON.stringify({id: 0, src: INIT_SRC, method: 'Shelly.GetStatus' }),
                                    qos: this.adapter.config.qos,
                                    messageId: this.messageId,
                                });
                            } catch (err) {
                ...
                this.adapter.log.error(`[MQTT] (Shelly?) device unknown, configuration for client with id "${packet.clientId}" does not exist!`);
                ...
                if (this.getDeviceGen() == 1) {
                    if (packet.topic === 'shellies/announce') {
                        try {
                            ...
                            const deviceMode = payloadObj?.mode;

                            // Device onboarding
                            if (!this.onboardingCompleted) {
                                const ip = payloadObj?.ip;
                                this.setIP(ip, 'Device onboarding (Gen 1)');
                                this.deviceMode = deviceMode;

                                await this.finishDeviceOnboarding();
                            } else if (deviceMode) {
                                await this.setDeviceMode(deviceMode);
                            }
                    ...
                    if (packet.topic === `${INIT_SRC}/rpc`) {
                        try {
                            ...
                            if (!this.onboardingCompleted) {
                                ...
                                if (ip) {
                                    this.setIP(ip, 'Device onboarding (Gen 2)');
