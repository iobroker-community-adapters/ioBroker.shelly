...
class ExternalMQTTDeviceClient extends MQTTClient {
    ...
    setMqttPrefixById(prefix) {
        ...
        if (this.mqttprefix && !this._prefixesMatch(this.mqttprefix, prefix)) {
            this.adapter.log.warn(
                `[MQTT External] Ignoring unexpected mqtt prefix update from "${this.mqttprefix}" to "${prefix}" for ${this.getLogInfo()}`,
            );
            return;
        }
    ...
    /**
     * Update the externally detected device prefix and MQTT prefix.
     *
     * @param {string} prefix - Device MQTT prefix
     */
    updateDevicePrefix(prefix) {
        if (!prefix || this.devicePrefix === prefix) {
            return;
        }

        this.devicePrefix = prefix;
        this.id = this.normalizeExternalDeviceId(prefix);
        this.setMqttPrefixById(prefix);
    }

    /**
     * Compare two device prefixes and treat folder-prefixed variants as equivalent.
     *
     * @param {string} left - First prefix
     * @param {string} right - Second prefix
     * @returns {boolean} True if both prefixes refer to the same device
     */
    _prefixesMatch(left, right) {
        return prefixesMatch(left, right);
    }
...
class MQTTServerExternal extends BaseServer {
    ...
    listen() {
        ...
        this.mqttClientConnection.on('message', async (topic, payload, packet) => {
            ...
            let { prefix: clientPrefix, client } = this._findClientByEquivalentPrefix(devicePrefix);
            if (client && clientPrefix !== devicePrefix) {
                this._updateClientPrefixMapping(clientPrefix, devicePrefix, client);
            }
    ...
    async _handleInitSrcMessage(topic, payload) {
        try {
            ...
            let { prefix: existingPrefix, client } = this._findClientByEquivalentPrefix(srcDevice);
            if (client && existingPrefix !== srcDevice) {
                this.adapter.log.debug(
                    `[MQTT External] INIT_SRC source "${srcDevice}" matched existing prefix "${existingPrefix}"`,
                );
                this._updateClientPrefixMapping(existingPrefix, srcDevice, client);
            }
            if (!client) {
    ...
    async _requestStatusFromKnownDevices() {
        try {
            ...
            for (const deviceId of deviceIds) {
                ...
                let { prefix: existingPrefix, client } = this._findClientByEquivalentPrefix(devicePrefix);
                if (!client) {
                    client = new ExternalMQTTDeviceClient(
                        this.adapter,
                        this.objectHelper,
                    ...
                    this._updateClientPrefixMapping(existingPrefix, devicePrefix, client);
        ...
        if (parts[0] === 'shellies') {
            if (parts.length >= 3 && parts[1] !== 'announce') {
                if (topic.endsWith('/online')) {
                    return topic.substring(SHELLIES_PREFIX.length, topic.length - ONLINE_SUFFIX.length);
                }

                const knownPrefix = this._findKnownGen1PrefixByTopic(topic);
                if (knownPrefix) {
                    return knownPrefix;
                }
            }
    ...
    /**
     * Find an already known Gen1 device prefix from an incoming topic.
     *
     * @param {string} topic - MQTT topic
     * @returns {string|null} Known device prefix or null
     */
    _findKnownGen1PrefixByTopic(topic) {
        let matchedPrefix = null;
        for (const prefix of Object.keys(this.deviceClients)) {
            if (topic === `shellies/${prefix}` || topic.startsWith(`shellies/${prefix}/`)) {
    ...
    /**
     * Find an already known client by exact or folder-equivalent MQTT prefix.
     *
     * @param {string} prefix - MQTT topic prefix
     * @returns {{prefix: string|null, client: ExternalMQTTDeviceClient|null}}
     */
    _findClientByEquivalentPrefix(prefix) {
        if (!prefix) {
            return { prefix: null, client: null };
        }

        if (this.deviceClients[prefix]) {
            return { prefix, client: this.deviceClients[prefix] };
        }

        for (const existingPrefix of Object.keys(this.deviceClients)) {
            if (this._prefixesMatch(existingPrefix, prefix)) {
                return { prefix: existingPrefix, client: this.deviceClients[existingPrefix] };
            }
        }
    ...
    /**
     * Compare two MQTT prefixes and treat plain and folder-prefixed variants as equivalent.
     *
     * @param {string} left - First prefix
     * @param {string} right - Second prefix
     * @returns {boolean} True if both prefixes refer to the same device
    ...
    /**
     * Update the stored client mapping to the newest observed MQTT prefix.
     *
     * @param {string|null} oldPrefix - Existing map key
     * @param {string} newPrefix - New map key
     * @param {ExternalMQTTDeviceClient} client - Device client
     */
    _updateClientPrefixMapping(oldPrefix, newPrefix, client) {
        if (!client || !newPrefix || oldPrefix === newPrefix) {
            return;
        }

        if (oldPrefix && this.deviceClients[oldPrefix] === client) {
            delete this.deviceClients[oldPrefix];
        }

        this.deviceClients[newPrefix] = client;
        client.updateDevicePrefix(newPrefix);
    }
    ...
    async _getStoredDevicePrefix(deviceId, fallbackPrefix) {
        try {
            ...
            const storedPrefix = typeof state?.val === 'string' ? state.val.trim() : '';
            if (storedPrefix) {
                return storedPrefix;
            }
        } catch (err) {
