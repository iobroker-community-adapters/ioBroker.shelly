...
class MQTTClient extends BaseClient {
    constructor(adapter, objectHelper, eventEmitter, stream) {
        ...
        this.initializing = false; // use to delay publish packates after new connection
    ...
    listener() {
        ...
        this.client.on('publish', async packet => {
            ...
            if (this.initializing) {
                let cnt = 5 * 60; // limit to one minute;
                this.adapter.log.debug(`[MQTT] Publish: ${this.getLogInfo()} - initialization in progress, waiting to process published data ...`);
                while (this.initializing && cnt--) {
                    this.adapter.delay(200);
                }
                if (cnt) {
                    this.adapter.log.debug(
                        `[MQTT] Publish: ${this.getLogInfo()} - initialization completed, continuing...`,
                    );
                } else {
                    this.adapter.log.debug(
                        `[MQTT] Publish: ${this.getLogInfo()} - initialization TIMEOUT, continuing...`,
                    );
                }
            }
