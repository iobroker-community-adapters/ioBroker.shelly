...
class MQTTClient extends BaseClient {
    ...
    listener() {
        ...
        this.adapter.log.debug(`[MQTT] Created listener`);
    }
}
...
class MQTTServer extends BaseServer {
    ...
    constructor(adapter, objectHelper, eventEmitter) {
        ...
        if (this.adapter.config.useMqtts) {
            this.adapter.log.debug(`[MQTT Server] Creating MQTT Server with TLS configuration`);

            this.server = tls.createServer({
                ...adapter.getMqttCertificates(),
                rejectUnauthorized: false,
            });
        } else {
            this.adapter.log.debug(`[MQTT Server] Creating MQTT Server`);
    ...
    listen() {
        this.server.on('connection', /** @param {net.Socket} stream */ (stream) => {
        ...
        if (this.adapter.config.useMqtts) {
            this.server.on('secureConnection', /** @param {tls.TLSSocket} stream */ (stream) => {
                this.adapter.log.debug(`[MQTT TLS Server] New secure connection from ${stream.remoteAddress} - authorized: ${stream.authorized}`);

                if (stream.authorizationError) {
                    this.adapter.log.error(`[MQTT TLS Server] Authorization error ${stream.authorizationError}`);
                }
            });

            this.server.on('keylog', (line, stream) => {
                this.adapter.log.debug(`[MQTT TLS Server] Keylog from ${stream.remoteAddress}: ${line}`);
            });

            this.server.on('newSession', (id, data, cb) => {
                this.tlsSessionStore[id.toString('hex')] = data;
                this.adapter.log.debug(`[MQTT TLS Server] Session started: ${id.toString('hex')}`);

                cb();
            });

            this.server.on('resumeSession', (id, cb) => {
                this.adapter.log.debug(`[MQTT TLS Server] Session resumed: ${id.toString('hex')}`);

                cb(null, this.tlsSessionStore[id.toString('hex')] || null);
            });

            this.server.on('tlsClientError', (error) => {
                this.adapter.log.error(`[MQTT TLS Server] client error: ${error}`);
            });
        }
