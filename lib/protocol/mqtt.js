...
class MQTTServerExternal extends BaseServer {
    ...
    listen() {
        ...
        this.mqttClientConnection.on('connect', async () => {
            ...
            // Subscribe to all topics to support custom (possibly multi-level) MQTT prefixes.
            this.mqttClientConnection.subscribe('#', { qos: 0 }, err => {
                if (err) {
                    this.adapter.log.error(`[MQTT External] Failed to subscribe to #: ${err}`);
                }
            });
