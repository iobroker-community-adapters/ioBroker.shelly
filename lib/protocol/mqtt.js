    ...
    listener() {
        ...
        this.client.on('publish', async (packet) => {
            ...
            if (packet.payload) {
                ...
                if (this.getDeviceGen() == 1 && packet.topic === 'shellies/announce') {
                    try {
                        ...
                        if (newDeviceMode && newDeviceMode !== this.getDeviceMode()) {
                            this.adapter.log.info(`[MQTT] Publish - device mode of ${this.getLogInfo()} changed from "${this.getDeviceMode()}" to "${newDeviceMode}" - recreating states NOW!`);
                ...
                // Generation 2
                if (this.getDeviceGen() == 2 && packet.topic == 'iobroker/rpc') {
                    try {
