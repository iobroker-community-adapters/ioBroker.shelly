            ...
            if (packet.payload) {
                ...
                if (packet.topic == 'iobroker/rpc') {
                    try {
                        ...
                        this.adapter.log.debug(`[MQTT] Error parsing original error: ${this.getLogInfo()} - topic: ${packet.topic}, error: ${err}`);
                    }
                }
            }
            ...
            if (packet.topic === 'shellies/announce' && packet.payload) {
                try {
                    ...
                    if (ip && ip !== this.ip) {
                        ...
                        this.adapter.setStateAsync(`${this.getDeviceId()}.hostname`, { val: this.getIP(), ack: true });
                        this.adapter.deviceStatusUpdate(this.getDeviceId(), true); // Device online
                    }

                    const newDeviceMode = payloadObj?.mode;
                    if (newDeviceMode && newDeviceMode !== this.getDeviceMode()) {
                        this.adapter.log.warn(`[MQTT] Publish - device mode changed from "${this.getDeviceMode()}" to "${newDeviceMode}" - recreating states NOW!`);
