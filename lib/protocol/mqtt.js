...
class MQTTServerExternal extends BaseServer {
    ...
    listen() {
        ...
        this.mqttClientConnection.on('message', async (topic, payload, packet) => {
            ...
            let retainedMessageHandledDuringInitialization = false;
            if (isOnlineTopic) {
            ...
            if (!client) {
                ...
                await this._queuePendingRetainedMessages(client, devicePrefix);
                retainedMessageHandledDuringInitialization = isRetained;
            }
            ...
            if (!retainedMessageHandledDuringInitialization) {
                try {
                    await client.handleMessage(topic, payload);
                } catch (err) {
                    this.adapter.log.error(
                        `[MQTT External] Error handling device packet for ${devicePrefix} on ${topic}: ${err?.message || err}`,
                    );
                }
            }
        });
        ...
        try {
            ...
            for (const deviceId of deviceIds) {
                ...
                if (!client) {
                    ...
                    this._queuePendingRetainedMessages(newClient, devicePrefix).catch(err => {
                        this.adapter.log.error(
    ...
    _rememberPendingRetainedMessage(devicePrefix, topic, payload) {
        ...
        this.pendingRetainedMessages[devicePrefix].set(topic, Buffer.from(payload));
    }

    async _queuePendingRetainedMessages(client, devicePrefix) {
