...
class MQTTClient extends BaseClient {
    ...
    async destroy() {
        // Process remaining queued packets before destroying
    ...
    listener() {
        ...
        this.client.on('publish', async packet => {
            ...
            // Queue packet during initialization, process immediately otherwise
            if (this.initializing) {
                this.adapter.log.debug(
                    `[MQTT] Publish: ${this.getLogInfo()} - initialization in progress, queueing packet for later processing`,
                );
                this.publishQueue.push(packet);
            } else {
                // Process packet immediately if not initializing
                await this.processPublishPacket(packet);
            }
        });
