...
const virtualHelper = require('../devices/virtual-helper');
...
class MQTTClient extends BaseClient {
    ...
    /**
     * process components packet
     * "result":{"components":[{"key":"ble","status":{},"config":{"enable":false,"rpc":{"enable":true}}},{"key":"bthome","status":{"errors":["bluetooth_disabled"]},"config":{}},{"key":"cloud","status":{"connected":true},"config":{"enable":true,"server":"shelly-82-eu.shelly.cloud:6022/jrpc"}},{"key":"input:0","status":{"id":0,"state":false},"config":{"id":0,"name":null,"type":"switch","enable":true,"invert":false,"factory_reset":true}},{"key":"knx","status":{},"config":{"enable":false,"ia":"15.15.255","routing":{"addr":"224.0.23.12:3671"}}},{"key":"matter","status":{"num_fabrics":0,"commissionable":false},"config":{"enable":true}},{"key":"modbus","status":{},"config":{"enable":true}},{"key":"mqtt","status":{"connected":true},"config":{"enable":true,"server":"10.17.2.16:1881","client_id":"shelly1minig4-7c2c67642750","user":"mqttuser","ssl_ca":null,"topic_prefix":"shelly1minig4-7c2c67642750","rpc_ntf":true,"status_ntf":true,"use_client_cert":false,"enable_rpc":true,"enable_control":true}},{"key":"switch:0","status":{"id":0, "source":"HTTP_in", "output":false,"temperature":{"tC":51.1, "tF":124.1}},"config":{"id":0, "name":null,"in_mode":"follow","in_locked":false,"initial_state":"match_input", "auto_on":false, "auto_on_delay":60.00, "auto_off":false, "auto_off_delay":60.00},"attrs":{"inputs":[0]}},{"key":"sys","status":{"mac":"7C2C67642750","restart_required":false,"time":"17:00","unixtime":1769356813,"last_sync_ts":1769356397,"uptime":68790,"ram_size":346872,"ram_free":177664,"ram_min_free":162800,"fs_size":917504,"fs_free":466944,"cfg_rev":13,"kvs_rev":0,"schedule_rev":0,"webhook_rev":0,"btrelay_rev":0,"bthc_rev":0,"available_updates":{"beta":{"version":"1.7.4-beta2"}},"alt":{"Mini1G4ZB":{"name":"Shelly Mini 1 Gen4","desc":"Shelly Mini 1 Gen4 with Zigbee","beta":{"version":"1.7.4-beta2","build_id":"20251218-123250/1.7.4-beta2-g9f7f41f"},"stable":{"version":"1.7.1","build_id":"20250924-062733/1.7.1-gd336f31"}}},"reset_reason":3,"utc_offset":3600},"config":{"device":{"name":null,"mac":"7C2C67642750","fw_id":"20250924-062714/1.7.1-gd336f31","discoverable":true,"eco_mode":false},"location":{"tz":"Europe/Vienna","lat":48.153500,"lon":16.385500},"debug":{"level":2,"file_level":null,"mqtt":{"enable":false},"websocket":{"enable":false},"udp":{"addr":null}},"ui_data":{},"rpc_udp":{"dst_addr":null,"listen_port":null},"sntp":{"server":"time.cloudflare.com"},"cfg_rev":13}}],"cfg_rev":13,"offset":0,"total":12}}
     *
     * @param payloadObj
     */
    async processPublishComponents(payloadObj) {
        const components = payloadObj.result?.components;
        if (!components) {
            return;
        }

        this.adapter.log.debug(`[MQTT] Components: ${this.getLogInfo()} - received ${components.length} components`);
        for (const component of components) {
            ...
            if (key.includes(':')) {
                this.adapter.log.debug(`[MQTT] Components: ${this.getLogInfo()} - processing component ${key}`);
                const compName = key.split(':')[0];
                const compId = Number(key.split(':')[1]);

                if (compId >= 0 && compId < 200) {
                    this.adapter.log.debug(
                        `[MQTT] Components: ${this.getLogInfo()} - component ${key} is a static component`,
                    );
                } else if (compId >= 200 && compId <= 299) {
                    this.adapter.log.debug(
                        `[MQTT] Components: ${this.getLogInfo()} - component ${key} is a virtual component`,
                    );
                    //await handleVirtualComponent(compName, compId, component.)
                    const testDummy = {};
                    virtualHelper.addVirtualNumber(testDummy, 201);
                    await this.addVirtualComponentState(testDummy);
                    await this.createObjects();
                } else {
                    this.adapter.log.debug(
                        `[MQTT] Components: ${this.getLogInfo()} - component ${key} - ${compId} is out of range`,
                    );
                }
            } else {
                this.adapter.log.debug(`[MQTT] Components: ${this.getLogInfo()} - ignoring component ${key}`);
            }
        }
    }
            ...
            while (this.publishQueue.length > 0) {
                ...
                try {
                    ...
                    this.adapter.log.error(
                        `[MQTT] Publish: ${this.getLogInfo()} - error processing packet: ${err && err.message ? err.message : err}`,
                    );
                }
            }
        } finally {
            ...
            if (this.getDeviceGen() >= 2) {
                ...
                if (packet.topic === `${INIT_SRC}/rpc`) {
                    try {
                        ...
                        // handle components
                        this.processPublishComponents(payloadObj);
                    } catch (err) {
                        this.adapter.log.debug(
                ...
                if (
                    ...
                    if (this.publishQueue.length > 0) {
                        ...
                        try {
                            ...
                            this.adapter.log.error(
                                `[MQTT] Error processing publish queue for ${this.getLogInfo()}: ${err && err.message ? err.message : err}`,
                            );
                        }
                    }
                } else {
