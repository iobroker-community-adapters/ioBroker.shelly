...
class MQTTClient extends BaseClient {
    ...
    publishStateValue(topic, value) {
        ...
        const qos = Number.isFinite(parsedQos) ? parsedQos : 0;
...
class ExternalMQTTDeviceClient extends MQTTClient {
    ...
    async destroy() {
        ...
        try {
            ...
            this.adapter.log.error(
                `[MQTT External] Error while destroying ${this.getLogInfo()}: ${error.message}`,
            );
        }
    }
}
    ...
    listen() {
        ...
        this.mqttClientConnection.on('connect', async () => {
            ...
            this.mqttClientConnection.subscribe('#', { qos: 0 }, err => {
                if (err) {
                    this.adapter.log.error(`[MQTT External] Failed to subscribe to #: ${err?.message || err}`);
                }
            });
        });
    ...
    publishToDevice(topic, value, qos) {
        if (this.mqttClientConnection && this.mqttClientConnection.connected) {
            this.mqttClientConnection.publish(topic, value, { qos: qos || 0 }, err => {
                if (err) {
                    this.adapter.log.debug(`[MQTT External] Error publishing to topic ${topic}: ${err?.message || err}`);
                }
            });
        } else {
