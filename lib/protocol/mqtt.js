...
class MQTTClient extends BaseClient {
    ...
    destroy() {
        ...
        for (const messageId in this.messageCache) {
            if (this.messageCache[messageId].resendid) clearTimeout(this.messageCache[messageId].resendid);
        }
    ...
    publishStateValue(topic, value) {
        ...
        const qosConfig = this.adapter.config.qos;
        const qos = qosConfig ? Number.parseInt(qosConfig) : 0;
                        ...
                        if (this.getDeviceGen() == 2) {
                            try {
                                ...
                                this.publishStateValue(
                                    '<mqttprefix>/rpc',
                                    JSON.stringify({id: 0, src: INIT_SRC, method: 'Shelly.GetDeviceInfo', params: { ident: true }}),
                                );

                                this.publishStateValue(
                                    '<mqttprefix>/rpc',
                                    JSON.stringify({id: 0, src: INIT_SRC, method: 'Shelly.GetStatus' }),
                                );
                            } catch (err) {
