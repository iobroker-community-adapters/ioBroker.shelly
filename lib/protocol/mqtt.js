...
class MQTTClient extends BaseClient {
    ...
    start() {
        if (!this.stream) {
            return; // External client mode - no stream needed
        }
...
class MQTTServerExternal extends BaseServer {
    ...
    listen() {
        ...
        const username = this.adapter.config.mqttClientUsername;
        const password = this.adapter.config.mqttClientPassword;
        ...
        this.mqttClientConnection.on('connect', () => {
        ...
        this.mqttClientConnection.on('message', async (topic, payload) => {
            ...
            if (!client) {
                ...
                client.initialize().catch(err => {
                    this.adapter.log.error(
                        `[MQTT External] Error initializing device ${devicePrefix}: ${err && err.message ? err.message : err}`,
                    );
                });
            }
        ...
        this.mqttClientConnection.on('error', err => {
            this.adapter.log.error(`[MQTT External] Connection error: ${err && err.message ? err.message : err}`);
        });
