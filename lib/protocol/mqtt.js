...
class MQTTClient extends BaseClient {
    ...
    async setMqttPrefixHttp() {
        ...
        this.adapter.log.debug(
            `[MQTT] Started mqttprefix fallback via HTTP for ${this.getLogInfo()} (Gen ${this.getDeviceGen()})`,
        );
        ...
        if (this.getDeviceGen() == 1) {
            try {
                ...
                if (body) {
                    ...
                    this.adapter.log.debug(
                        `[MQTT] Requested mqttprefix for ${this.getLogInfo()} (Gen 1): ${this.mqttprefix}`,
                    );
                ...
                if (err && err.response && err.response.status == 401) {
                    this.adapter.log.error(
                        `[MQTT] Wrong http username or http password! Please enter the user credential from restricted login for ${this.getLogInfo()}`,
                    );
                } else {
                    this.adapter.log.error(
                        `[MQTT] Error in function setMqttPrefixHttp (Gen 1) for ${this.getLogInfo()}: ${err}`,
                    );
                }
            }
            ...
            try {
                ...
                if (body) {
                    ...
                    this.adapter.log.debug(
                        `[MQTT] Requested mqttprefix for ${this.getLogInfo()} (Gen 2+): ${this.mqttprefix}`,
                    );
                ...
                this.adapter.log.error(
                    `[MQTT] Error in function setMqttPrefixHttp (Gen 2+) for ${this.getLogInfo()}: ${err}`,
                );
            }
        }
    ...
    listener() {
        ...
        this.client.on('connect', async packet => {
            ...
            if (this.deviceExists()) {
                ...
                this.adapter.log.error(
                    `[MQTT] (Shelly?) device unknown, configuration for client with id "${packet.clientId}" ${this.getLogInfo()} does not exist! Maybe this device is not supported in this adapter version.`,
                );
                this.adapter.log.error(
