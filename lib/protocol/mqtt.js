...
const utils = require('../mqttDeviceUtils');
...
class MQTTClient extends BaseClient {
    ...
    const id = this.getId();
    if (!id) {
        this.adapter.log.error(`[MQTT] Client ID is undefined in setMqttPrefixHttp.`);
        return undefined;
    }

    const gen = this.getDeviceGen();
    this.adapter.log.debug(
        `[MQTT] Started mqttprefix fallback via HTTP or RPC for client ${id} (Gen ${gen})`,
    );
    ...
    if (gen === 1) {
        try {
            ...
            if (body) {
                ...
                if (!settings?.mqtt?.id) {
                    this.adapter.log.error(`[MQTT] Settings response missing mqtt.id for client ${id}`);
                    return undefined;
                }
                this.mqttprefix = settings.mqtt.id;

                this.adapter.log.debug(
                    `[MQTT] Requested mqttprefix for client ${id} (Gen 1): ${this.mqttprefix}`,
                );
            ...
            if (err && err.response && err.response.status === 401) {
                this.adapter.log.error(
                    `[MQTT] Wrong http username or http password! Please enter the user credential from restricted login for client ${id}`,
                );
            } else {
                this.adapter.log.error(
                    `[MQTT] Error in function setMqttPrefixHttp (Gen 1) for client ${id}: ${err}`,
                );
            }
        }
    } else if (gen >= 2) {
        try {
            ...
            if (body) {
                ...
                if (!settings?.topic_prefix) {
                    this.adapter.log.error(`[MQTT] Settings response missing topic_prefix for client ${id}`);
                    return undefined;
                }
                this.mqttprefix = settings.topic_prefix;

                this.adapter.log.debug(
                    `[MQTT] Requested mqttprefix for client ${id} (Gen 2+): ${this.mqttprefix}`,
                );
            ...
            this.adapter.log.error(
                `[MQTT] Error in function setMqttPrefixHttp (Gen 2+) for client ${id}: ${err}`,
            );
        }
    }
