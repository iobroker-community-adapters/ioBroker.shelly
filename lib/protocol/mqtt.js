...
const SHELLIES_PREFIX = 'shellies/';
...
class MQTTClient extends BaseClient {
    ...
    static _registerRun(self) {
        if (self) {
            ...
            const oldClient = name ? this.clientlist[name] : undefined;
            // Register new client first so destroy() of old client can detect the replacement
            this.clientlist[name] = self;
            if (oldClient && oldClient !== self) {
                oldClient.destroy();
            }
        }
    }
    ...
    getSerialId() {
        if (!this.serialId) {
            let id = this.getId();
            if (id) {
                id = this._getDevicePrefixSegment(id);
    ...
    async processPublishPacket(packet) {
        ...
        if (packet.payload) {
            ...
            if (this.getDeviceGen() >= 2) {
                ...
                if (
                    packet.topic === `${this.getMqttPrefix()}/status/wifi` ||
                    packet.topic === `${this.getMqttPrefix()}/status/eth`
                ) {
                    try {
                        const payloadObj = JSON.parse(packet.payload.toString());
                        const ip = payloadObj?.sta_ip || payloadObj?.ip;
                        if (ip && ip !== this.getIP()) {
                            await this.setIP(ip, `Gen 2+ ${packet.topic}`);
                            await this.adapter.deviceStatusUpdate(this.getDeviceId(), true);

                            if (!this.httpIoBrokerStateDone) {
                                this.httpIoBrokerStateDone = true;
                                await this.httpIoBrokerState();
                            }
                        }
                    } catch (err) {
                        this.adapter.log.debug(
                            `[MQTT] Error parsing wifi/eth status: ${this.getLogInfo()} - error: ${err}`,
                        );
                    }
                }
    ...
    async destroy() {
        ...
        // If a newer client has already registered for this device, skip setting it offline.
        // This prevents a race condition where the old client's async destroy() completes
        // after the new client has already brought the device back online.
        const ownId = this.getId();
        const isReplaced =
            ownId && MQTTClient.clientlist?.[ownId] !== undefined && MQTTClient.clientlist[ownId] !== this;
        if (isReplaced) {
            this.adapter.log.debug(
                `[MQTT] Destroying ${this.getLogInfo()} - replaced by newer client, skipping offline status`,
            );
        }

        super.destroy(/* setOffline: */ !isReplaced);
    ...
    setMqttPrefixByWill(topic) {
        ...
        if (topic) {
            ...
            if (this.getDeviceGen() === 1) {
                if (arr[0] === 'shellies' && arr.length >= 3) {
                    this.mqttprefix = topic.substring(SHELLIES_PREFIX.length, topic.length - ONLINE_SUFFIX.length);
...
class ExternalMQTTDeviceClient extends MQTTClient {
    ...
    /**
     * Return the external MQTT topic prefix as device id.
     * External broker mode expects the same single-segment topic prefix shape as the internal broker.
     *
     * @param {string} prefix - Full MQTT topic prefix
     * @returns {string} MQTT topic prefix
     */
     ...
     */
    async initialize() {
        this.httpIoBrokerStateDone = false;
        if (!this.deviceExists()) {
            this.adapter.log.error(
        ...
        await this.initIPFromState();
        this.adapter.log.debug(`[DEBUG] ip=${this.getIP()}, online=${this.isOnline()}, deviceId=${this.getDeviceId()}`);
        await this.adapter.deviceStatusUpdate(this.getDeviceId(), true);
        this.adapter.log.debug(`[DEBUG] after deviceStatusUpdate: online=${this.isOnline()}`);
    ...
    setMqttPrefixById(prefix) {
        if (!prefix || this.mqttprefix === prefix) {
            return;
        }
...
class MQTTServerExternal extends BaseServer {
    constructor(adapter, objectHelper, eventEmitter) {
        ...
        this.pendingOnlineStates = {}; // devicePrefix -> latest broker online flag
        this.pendingRetainedMessages = {}; // devicePrefix -> retained device packets seen before client exists
    }
    ...
    listen() {
        ...
        this.mqttClientConnection.on('connect', async () => {
            ...
            const topics = [
                'shellies/#',
                // Gen2+ devices with default bare prefix (<id>/...)
                '+/events/#',
                '+/status/#',
                '+/rpc',
                '+/online',
                '+/debug/#',
                // Gen2+ devices with shelly/ folder prefix (shelly/<id>/...)
                'shelly/+/events/#',
                'shelly/+/status/#',
                'shelly/+/rpc',
                'shelly/+/online',
            ...
            await this._subscribeToExternalTopics(topics);
        ...
        this.mqttClientConnection.on('message', async (topic, payload, packet) => {
            ...
            const previousOnlineState = isOnlineTopic ? this.pendingOnlineStates[devicePrefix] : undefined;
            const isOnline = isOnlineTopic ? this._isOnlinePayload(payloadString) : false;
            let retainedHandledOnInit = false;
            if (isOnlineTopic) {
                this._rememberOnlineState(devicePrefix, isOnline);
            }
            ...
            if (!client) {
                if (isRetained) {
                    this._rememberPendingRetainedMessage(devicePrefix, topic, payload);
                }
                ...
                await this._replayPendingRetainedMessages(client, devicePrefix);
                retainedHandledOnInit = isRetained;
            }

            if (client && isOnlineTopic) {
                await this._handleKnownDeviceOnlineStateChange(client, devicePrefix, previousOnlineState, isOnline);
            }

            if (!retainedHandledOnInit) {
    ...
    async _handleInitSrcMessage(topic, payload) {
        try {
            ...
            const ip = payloadObj?.result?.eth?.ip || payloadObj?.result?.wifi?.sta_ip;
            if (ip && client && ip !== client.getIP()) {
                await client.setIP(ip, `Gen 2+ ${INIT_SRC}/rpc external`);
                await this.adapter.deviceStatusUpdate(client.getDeviceId(), true);
    ...
    /**
     * Subscribe to all external MQTT topics and wait for broker acknowledgements
     * before requesting device status.
     *
     * @param {string[]} topics
     */
    async _subscribeToExternalTopics(topics) {
        const subscribeJobs = topics.map(
            topic =>
                new Promise(resolve => {
        ...
        try {
            await Promise.all(subscribeJobs);
        } catch (err) {
            this.adapter.log.warn(
                `[MQTT External] Could not await all topic subscriptions: ${err?.message || String(err)}`,
            );
        }
    }
    ...
    async _requestStatusFromKnownDevices() {
        try {
            ...
            for (const deviceId of deviceIds) {
                ...
                // Fallback to bare device class prefix (Shelly default MQTT topic prefix)
                const fallbackPrefix = `${deviceClass}-${serialId}`;
                const devicePrefix = await this._getStoredDevicePrefix(deviceId, fallbackPrefix);
                const client = this.deviceClients[devicePrefix];
                if (!client) {
                    const newClient = new ExternalMQTTDeviceClient(
                        this.adapter,
                        this.objectHelper,
                        this.eventEmitter,
                        devicePrefix,
                    ...
                    this.deviceClients[devicePrefix] = newClient;
                    newClient
                        .initialize()
                        .then(async () => {
                            await this._applyPendingOnlineState(newClient, devicePrefix);
                            this._sendStatusRequest(newClient);
                        })
                        .catch(err => {
                            this.adapter.log.error(
                    ...
                    this._replayPendingRetainedMessages(newClient, devicePrefix).catch(err => {
                        this.adapter.log.error(
                            `[MQTT External] Error replaying retained packets for ${devicePrefix}: ${err?.message || err}`,
                        );
                    });
                } else {
    ...
    _identifyDevicePrefix(topic) {
        ...
        // External broker mode supports:
        // - Gen1: shellies/<prefix>/...
        // - Gen2+ with shelly/ folder prefix: shelly/<prefix>/events|status|rpc|online|debug
        // - Gen2+ with bare (default) prefix: <prefix>/events|status|rpc|online|debug
        if (parts[0] === 'shellies' && parts.length >= 3 && this._isValidDevicePrefix(parts[1])) {
            return parts[1];
        }

        if (parts[0] === 'shelly' && parts.length >= 3 && this._isValidDevicePrefix(parts[1])) {
            ...
            if (
                ...
                suffix.startsWith('debug/')
            ) {
                return `shelly/${parts[1]}`;
            }
        }

        if (this._isValidDevicePrefix(parts[0])) {
    ...
    _isValidDevicePrefix(prefix) {
        if (!this._isSingleSegmentPrefix(prefix)) {
            return false;
        }

        const match = DEVICE_PREFIX_PATTERN.exec(prefix);
        return !!match && !!datapoints.getDeviceTypeByClass(match[1]);
    }

    _isSingleSegmentPrefix(prefix) {
        return typeof prefix === 'string' && prefix.length > 0 && !prefix.includes('/');
    }

    async _updateKnownDeviceOnlineState(client, isOnline) {
        ...
        try {
            await this.adapter.deviceStatusUpdate(deviceId, isOnline);
        } catch (err) {
            this.adapter.log.warn(
                `[MQTT External] Could not update online state for ${deviceId}: ${err?.message || err}`,
            );
        }
    }

    async _handleKnownDeviceOnlineStateChange(client, devicePrefix, previousOnlineState, isOnline) {
        if (previousOnlineState === false && isOnline) {
            await this._reinitializeKnownDeviceClient(client, devicePrefix);
            return;
        }

        await this._updateKnownDeviceOnlineState(client, isOnline);
    }

    async _reinitializeKnownDeviceClient(client, devicePrefix) {
        if (!client || client.initializing) {
            return;
        }

        this.adapter.log.info(
            `[MQTT External] Device "${devicePrefix}" reported online after offline state - reinitializing device client`,
        );
        ...
        try {
            ...
            this.adapter.log.error(
                `[MQTT External] Error reinitializing device ${devicePrefix}: ${err?.message || err}`,
            );
        }
    }

    _rememberOnlineState(devicePrefix, isOnline) {
        if (!devicePrefix) {
            return;
        }
        this.pendingOnlineStates[devicePrefix] = isOnline;
    }

    _rememberPendingRetainedMessage(devicePrefix, topic, payload) {
        if (!devicePrefix || !topic || payload == null) {
            return;
        }

        if (!this.pendingRetainedMessages[devicePrefix]) {
            this.pendingRetainedMessages[devicePrefix] = new Map();
        }

        this.pendingRetainedMessages[devicePrefix].set(
            topic,
            Buffer.isBuffer(payload) ? Buffer.from(payload) : Buffer.from(String(payload)),
        );
    }

    async _replayPendingRetainedMessages(client, devicePrefix) {
        const retainedMessages = this.pendingRetainedMessages[devicePrefix];
        if (!retainedMessages) {
            return;
        }

        delete this.pendingRetainedMessages[devicePrefix];

        for (const [topic, payload] of retainedMessages.entries()) {
            try {
                await client.handleMessage(topic, payload);
            } catch (err) {
                this.adapter.log.warn(
                    `[MQTT External] Could not replay retained packet for ${devicePrefix} on ${topic}: ${err?.message || err}`,
                );
            }
        }
    }

    async _applyPendingOnlineState(client, devicePrefix) {
        if (!devicePrefix || !Object.prototype.hasOwnProperty.call(this.pendingOnlineStates, devicePrefix)) {
            return;
        }

        await this._updateKnownDeviceOnlineState(client, this.pendingOnlineStates[devicePrefix]);
    }

    /**
     * Read the persisted MQTT topic prefix of a known device.
     *
     * @param {string} deviceId - ioBroker device id
     * @param {string} fallbackPrefix - Prefix derived from the device id
     * @returns {Promise<string>} Stored or derived prefix
     */
    async _getStoredDevicePrefix(deviceId, fallbackPrefix) {
        try {
            const state = await this.adapter.getStateAsync(`${deviceId}.Mqtt.topicPrefix`);
            const storedPrefixValue = typeof state?.val === 'string' ? state.val.trim() : '';
            if (this._isShellyFolderPrefix(storedPrefixValue)) {
                return storedPrefixValue;
            }
            if (this._isValidDevicePrefix(storedPrefixValue)) {
                return storedPrefixValue;
            }
        } catch (err) {
            this.adapter.log.debug(
                `[MQTT External] Could not read stored topic prefix for ${deviceId}: ${err?.message || err}`,
            );
        }
    ...
    _isShellyFolderPrefix(prefix) {
        if (typeof prefix !== 'string' || prefix.length === 0) {
            return false;
        }

        const parts = prefix.split('/');
        return parts.length === 2 && parts[0] === 'shelly' && this._isValidDevicePrefix(parts[1]);
    }
    /**
