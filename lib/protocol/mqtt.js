...
class ExternalMQTTDeviceClient extends MQTTClient {
    ...
    async initialize() {
        ...
        this.adapter.log.info(`[MQTT External] Device ${this.getLogInfo()} initialized.`);
        this.initializing = false;
...
class MQTTServerExternal extends BaseServer {
    ...
    listen() {
        ...
        const port = parseInt(this.adapter.config.mqttClientPort, 10) || 1883;
        ...
        this.adapter.log.info(`[MQTT External] Connecting to external MQTT broker at ${brokerUrl}:${port}`);

        this.mqttClientConnection = mqttClient.connect(`${brokerUrl}:${port}`, options);

        this.mqttClientConnection.on('connect', async () => {
            this.adapter.log.info(`[MQTT External] Connected to external MQTT broker at ${brokerUrl}:${port}`);

            // Gen1 devices publish under shellies/<prefix>/...
            this.mqttClientConnection.subscribe('shellies/#', { qos: 0 }, err => {
                if (err) {
                    this.adapter.log.error(`[MQTT External] Failed to subscribe to shellies/#: ${err}`);
                }
            });

            // Gen2+ devices publish under <prefix>/events/rpc, <prefix>/rpc, <prefix>/online, etc.
            this.mqttClientConnection.subscribe('+/events/#', { qos: 0 });
            this.mqttClientConnection.subscribe('+/rpc', { qos: 0 });
            this.mqttClientConnection.subscribe('+/online', { qos: 0 });
            this.mqttClientConnection.subscribe('+/debug/#', { qos: 0 });

            // Receive RPC responses: Shelly devices reply to our commands on INIT_SRC/rpc
            this.mqttClientConnection.subscribe(`${INIT_SRC}/#`, { qos: 0 });

            // Devices connected to an external broker do not re-send their state when the
            // adapter (re)connects. Actively poll all known devices so the adapter is in sync.
            await this._requestStatusFromKnownDevices();
        });

        this.mqttClientConnection.on('message', async (topic, payload) => {
            if (this.adapter.isUnloaded) {
                return;
            }
    ...
    async _handleInitSrcMessage(topic, payload) {
        try {
            ...
            if (srcDevice && this.deviceClients[srcDevice]) {
                await this.deviceClients[srcDevice].handleMessage(topic, payload);
            }
        } catch (err) {
            this.adapter.log.debug(`[MQTT External] Malformed RPC response on ${topic}: ${err?.message || err}`);
        }
    }
     ...
     *
     * Devices connected to an external MQTT broker do NOT re-announce themselves when the
     * adapter restarts – only the adapter-to-broker connection drops and reconnects. Without
     * active polling the adapter would have no state until each device happens to publish a
     * value on its own.
     *
     * For each known device:
     * - If no in-memory client exists yet (adapter restart): create and initialize it.
     *   initialize() sets the MQTT prefix and sends Shelly.GetStatus / status_update.
     * - If a client already exists (network glitch reconnect): send a lightweight status
     *   request without going through the full initialization flow again.
     */
    async _requestStatusFromKnownDevices() {
        try {
            ...
            for (const deviceId of deviceIds) {
                ...
                let client = this.deviceClients[devicePrefix];
                if (!client) {
                    // Adapter restart: no in-memory client yet – create and initialize it.
                    client = new ExternalMQTTDeviceClient(
                        this.adapter,
                        this.objectHelper,
                        this.eventEmitter,
                    ...
                    // Reconnect after network glitch: client already in memory – re-request status.
     ...
     * Used on reconnect when the client is in memory but device state may have drifted.
     *
     * @param {ExternalMQTTDeviceClient} client - The device client to poll
     */
    _sendStatusRequest(client) {
        if (!client.getMqttPrefix()) {
            return; // prefix not yet known – skip until first message arrives
        }
        try {
            ...
            // status_update works for both Gen1 (primary) and Gen2+ (optional extra refresh)
            client.publishStateValue('<mqttprefix>/command', 'status_update');
        } catch (err) {
            this.adapter.log.debug(`[MQTT External] Error sending status request to ${client.getLogInfo()}: ${err}`);
        }
    }
    ...
    _identifyDevicePrefix(topic) {
        ...
        const gen2TopicParts = ['events', 'rpc', 'online', 'debug'];
        if (parts.length >= 2 && gen2TopicParts.includes(parts[1])) {
            return parts[0];
        }
    ...
    publishToDevice(topic, value, qos) {
        if (this.mqttClientConnection && this.mqttClientConnection.connected) {
            this.mqttClientConnection.publish(topic, value, { qos: qos || 0 }, err => {
                if (err) {
                    this.adapter.log.debug(`[MQTT External] Error publishing to topic ${topic}: ${err}`);
                }
            });
        } else {
