...
class MQTTClient extends BaseClient {
    ...
    /**
     * Shelly device type with the serial number of the device
    ...
    async createIoBrokerState(topic, payload) {
        ...
        for (const i in dps) {
            ...
            try {
                ...
                this.adapter.log.error(`[MQTT] Error ${error} in function dp.mqtt.mqtt_publish_funct of state ${fullStateId} for ${this.getLogInfo()}`);
            }
        }
        ...
        this.client.on('connect', async (packet) => {
            ...
            if (this.deviceExists()) {
                if (packet.username === this.adapter.config.mqttusername && packet.password !== undefined && packet.password.toString() === this.adapter.config.mqttpassword) {
                    ...
                    this.adapter.log.debug(`[MQTT] All objects created for ${this.getLogInfo()}`);
                    ...
                    if (ip !== '172.17.0.1') { // Docker
                        await this.setIP(ip, 'MQTT connect');
                    } else {
                        this.adapter.log.info(`Looks like you are running ioBroker in a Docker environment - trying to get IP of Shelly device via MQTT`);
                    }
                ...
                if (this.getDeviceGen() == 2) {
                    ...
                    if (packet.topic == `${INIT_SRC}/rpc`) {
                        try {
                            ...
                            // Update IP if not set (e.g. in Docker env)
                            if (!this.getIP()) {
                                const ip = payloadObj?.result?.eth?.ip || payloadObj?.result?.wifi?.sta_ip;
                                if (ip && ip !== this.getIP()) {
                                    this.setIP(ip, 'Gen 2 iobroker-init/rpc');
                                    this.adapter.deviceStatusUpdate(this.getDeviceId(), true); // Device online
                                }
                            }
