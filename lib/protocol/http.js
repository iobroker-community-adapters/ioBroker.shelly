...
function truncateLogMessage(value, maxLength = 500) {
    const text = sanitizeLogMessage(value);
...
function buildHttpRequestConfig(ip, url, timeout) {
    return {
        method: 'get',
        responseType: 'text',
...
function getWwwAuthenticateHeader(headers = {}) {
    return headers['www-authenticate'] || headers['WWW-Authenticate'];
}

function splitAuthenticateChallenges(header) {
    ...
    for (let index = 0; index < text.length; index++) {
        ...
        if (!inQuote && char === ',' && /\s*(Basic|Digest)\s+/i.test(text.slice(index + 1))) {
...
function parseAuthenticateChallenge(challenge) {
    const match = /^\s*(Basic|Digest)\s*(.*)$/i.exec(challenge || '');
    if (!match) {
        return undefined;
    }
    ...
    let paramMatch;
    while ((paramMatch = paramRegex.exec(paramsText))) {
        const rawValue = paramMatch[2].trim();
        params[paramMatch[1].toLowerCase()] =
            rawValue.startsWith('"') && rawValue.endsWith('"') ? rawValue.slice(1, -1).replace(/\\"/g, '"') : rawValue;
    }
...
function parseWwwAuthenticate(header) {
    return splitAuthenticateChallenges(header).map(parseAuthenticateChallenge).filter(Boolean);
}

function selectAuthChallenge(header) {
    const challenges = parseWwwAuthenticate(header);
    return (
        challenges.find(challenge => challenge.scheme === 'digest') ||
        challenges.find(challenge => challenge.scheme === 'basic')
    );
}
...
function getDigestAlgorithm(algorithm) {
    ...
    if (normalized === 'SHA-256-SESS' || normalized === 'SHA256-SESS') {
        return {
            name: 'SHA-256-SESS',
            nodeName: 'sha256',
        };
...
function getDigestAuthHeader({ method, url, username, password, challenge, nonceCount }) {
    const authDetails = challenge.params;
    const algorithm = getDigestAlgorithm(authDetails.algorithm);
    const qop = String(authDetails.qop || '')
        .split(',')
        .map(value => value.trim())
        .find(value => value === 'auth');
    const nc = `00000000${nonceCount}`.slice(-8);
    const cnonce = crypto.randomBytes(16).toString('hex');
    const hash = str => crypto.createHash(algorithm.nodeName).update(str).digest('hex');
    let HA1 = hash(`${username}:${authDetails.realm}:${password}`);

    if (algorithm.name.endsWith('-SESS')) {
        HA1 = hash(`${HA1}:${authDetails.nonce}:${cnonce}`);
    }

    const HA2 = hash(`${method}:${url}`);
    const response = qop
        ? hash(`${HA1}:${authDetails.nonce}:${nc}:${cnonce}:${qop}:${HA2}`)
        : hash(`${HA1}:${authDetails.nonce}:${HA2}`);
    const fields = [
        ['username', username],
    ...
    if (authDetails.opaque) {
        fields.push(['opaque', authDetails.opaque]);
    }
    if (qop) {
    ...
    return `Digest ${fields
        .map(([key, value, quoted = true]) => `${key}=${quoted ? `"${quoteDigestValue(value)}"` : value}`)
        .join(', ')}`;
}

function getAuthCache(nonceState) {
    if (!nonceState.authCache) {
        nonceState.authCache = {};
    }
    return nonceState.authCache;
}

function getCachedAuthScheme(nonceState, ip) {
    return getAuthCache(nonceState)[ip]?.scheme;
}

function setCachedAuthScheme(nonceState, ip, scheme, challenge) {
    getAuthCache(nonceState)[ip] = {
        scheme,
        challenge,
    };
    nonceState.logHttpAuth?.({ ip, scheme });
}

function buildAuthenticatedRequestConfig(ip, url, timeout, auth, nonceState, challenge) {
    const baseConfig = buildHttpRequestConfig(ip, url, timeout);
    const scheme = challenge?.scheme || getCachedAuthScheme(nonceState, ip);
    ...
    if (scheme === 'basic') {
        return applyBasicAuth(baseConfig, auth);
    }

    if (scheme === 'digest') {
        nonceState.nonceCount = (nonceState.nonceCount || 0) + 1;
        return {
            ...baseConfig,
            headers: {
                Authorization: getDigestAuthHeader({
                    ...
                    username: auth.username,
                    password: auth.password,
                    challenge: challenge || getAuthCache(nonceState)[ip]?.challenge,
...
async function performHttpRequest(ip, url, timeout, auth, nonceState = { nonceCount: 0 }) {
    let requestConfig = buildAuthenticatedRequestConfig(ip, url, timeout, auth, nonceState);
    const started = Date.now();
    nonceState.logHttpCommandRequest?.({
        method: 'GET',
        url,
        auth,
    });
    ...
    try {
        ...
        nonceState.logHttpCommandResponse?.({
            method: 'GET',
            url,
        ...
        nonceState.logHttpPollingResponse?.(url, response.status, Date.now() - started, response.data);
        return response.data;
    } catch (err) {
        if (auth && err?.response?.status === 401) {
            const challenge = selectAuthChallenge(getWwwAuthenticateHeader(err.response.headers));
            if (challenge) {
                setCachedAuthScheme(nonceState, ip, challenge.scheme, challenge);

                try {
                    requestConfig = buildAuthenticatedRequestConfig(ip, url, timeout, auth, nonceState, challenge);
                    const response = await axios(requestConfig);
                    nonceState.logHttpCommandResponse?.({
                        method: 'GET',
                        url,
                        ...
                        durationMs: Date.now() - started,
                        body: response.data,
                        authRetry: challenge.scheme,
                    });
                    nonceState.logHttpPollingResponse?.(url, response.status, Date.now() - started, response.data);
                    return response.data;
                } catch (retryErr) {
                    nonceState.logHttpAuthFailed?.({
        ...
        nonceState.logHttpCommandError?.({
            method: 'GET',
            url,
        ...
        nonceState.logHttpPollingResponse?.(
            url,
            err?.response?.status,
            Date.now() - started,
...
async function testHttpDeviceConnection(adapter, deviceConfig = {}) {
    ...
    const nonceState = {
        nonceCount: 0,
        logHttpAuth: ({ ip, scheme }) =>
            adapter.log.debug(`[HTTP] Test auth scheme detected for ${ip}: scheme=${scheme}`),
        logHttpAuthFailed: ({ scheme, endpoint, status }) =>
            adapter.log.debug(
                `[HTTP] Test auth failed: endpoint=${sanitizeLogMessage(endpoint)}, scheme=${scheme || '<unknown>'}, status=${status ?? '<none>'}`,
            ),
    };
    const result = {
        reachable: false,
...
function mergeHttpDevices(configuredDevices, discoveredDevices, autoCreate) {
    ...
    for (const device of [...(autoCreate ? discoveredDevices : []), ...configuredDevices]) {
        const key = getHttpDeviceMergeKey(device);
        const existing = devicesByKey.get(key);
        if (
            !existing ||
            device.source === 'manual' ||
            (device.source === 'ioBroker-registry' && existing.source !== 'manual')
        ) {
...
function getHttpDeviceMergeKey(device) {
    return device?.deviceId || device?.mac || device?.ip;
}
...
class HTTPPollingClient extends BaseClient {
    ...
    getHttpProtocolState(state, stateId) {
        const httpState = super.getHttpProtocolState(state, stateId);
        if (httpState?.mqtt_cmd && !httpState.http_cmd_funct && httpState.mqtt_cmd_funct) {
            return {
                ...httpState,
                http_cmd_funct: httpState.mqtt_cmd_funct,
            };
        }
        if (httpState?.http_publish || httpState?.http_cmd || httpState?.mqtt_cmd) {
            return httpState;
        }

        return (
            this.getSyntheticHttpCommandState(state, stateId) ||
            this.getGen1StatusProtocolState(state, stateId) ||
            httpState
        );
    }

    hasHttpControlCommand(httpState) {
        return !!(httpState?.http_cmd || httpState?.mqtt_cmd);
    }

    shouldRegisterHttpControlFunction(_state, _stateId, _hasTypeCmd, httpState) {
        return this.hasHttpControlCommand(httpState);
    }

    shouldRegisterMqttControlFunction() {
        return false;
    }

    shouldCreateHttpPollingState(_state, _stateId, _hasTypePublishCmd) {
        if (String(_stateId || '').startsWith('Mqtt.')) {
            return false;
        }
        return true;
    }

    isHttpCommandRequestDebugEnabled() {
        return this.adapter?.config?.httpDebugCommands === true;
    }

    isHttpRawResponseDebugEnabled() {
        return this.adapter?.config?.httpDebugRawResponses === true;
    }

    logHttpCommandRequest({ method, url, auth }) {
        if (!this.isHttpCommandRequestDebugEnabled()) {
            return;
        }
        this.adapter.log.debug(
            `[HTTP Command Debug] request started: method=${method}, url=${sanitizeLogMessage(url)}, auth=${
                auth ? `${auth.source || 'configured'} basic/digest enabled` : 'none'
            }`,
        );
    }

    logHttpCommandResponse({ method, url, status, durationMs, body, authRetry }) {
        if (!this.isHttpCommandRequestDebugEnabled()) {
            return;
        }
        this.adapter.log.debug(
            `[HTTP Command Debug] request finished: method=${method}, url=${sanitizeLogMessage(
                url,
            )}, status=${status}, responseTimeMs=${durationMs}, authRetry=${authRetry || 'none'}, body=${truncateLogMessage(
                body,
            )}`,
        );
    }

    logHttpCommandError({ method, url, status, durationMs, code, message, body }) {
        if (!this.isHttpCommandRequestDebugEnabled()) {
            return;
        }
        const authHint = status === 401 || status === 403 ? ', authError=true' : '';
        const timeoutHint = code === 'ECONNABORTED' || /timeout/i.test(String(message)) ? ', timeout=true' : '';
        this.adapter.log.debug(
            `[HTTP Command Debug] request failed: method=${method}, url=${sanitizeLogMessage(
                url,
            )}, status=${status ?? '<none>'}, responseTimeMs=${durationMs}, code=${code || '<none>'}${authHint}${timeoutHint}, message=${truncateLogMessage(
                message,
            )}, body=${truncateLogMessage(body || '')}`,
        );
    }

    logHttpAuth({ ip, scheme }) {
        this.adapter.log.debug(`[HTTP] Auth scheme detected for ${ip}: scheme=${scheme}`);
    }

    logHttpAuthFailed({ scheme, endpoint, status }) {
        this.adapter.log.debug(
            `[HTTP] Auth failed for ${this.getIP()} endpoint=${sanitizeLogMessage(endpoint)}, scheme=${scheme || '<unknown>'}, status=${status ?? '<none>'}`,
        );
    }

    getGen1StatusProtocolState(state, stateId) {
        if (this.getDeviceGen() !== 1 || !stateId) {
            return undefined;
        }

        const match = /^(Relay|Light)(\d+)\.(Power|Energy)$/.exec(stateId);
        if (!match) {
            return undefined;
        }

        const meterIndex = Number(match[2]);
        const meterField = match[3] === 'Power' ? 'power' : 'total';
        const publishFunction = state?.coap?.coap_publish_funct || state?.mqtt?.mqtt_publish_funct;

        return {
            http_publish: '/status',
            http_publish_funct: value => {
                const meterValue = parseJson(value, {})?.meters?.[meterIndex]?.[meterField];
                if (meterValue === undefined || meterValue === null) {
                    return undefined;
                }
                return publishFunction ? publishFunction(meterValue, this) : meterValue;
            },
        };
    }

    getSyntheticHttpCommandState(state, stateId) {
        if (!state?.common?.write || !stateId) {
            return undefined;
        }

        const relaySwitchMatch = /^Relay(\d+)\.(Switch|status)$/.exec(stateId);
        if (relaySwitchMatch) {
            const relayId = Number(relaySwitchMatch[1]);
            if (this.getDeviceGen() === 1) {
                return {
                    http_cmd: `/relay/${relayId}`,
                    http_cmd_funct: value => ({ turn: value ? 'on' : 'off' }),
                };
            }
            ...
            return {
                ...
                http_cmd_funct: value =>
                    JSON.stringify({
                        ...
                        src: 'iobroker',
                        method: 'Switch.Set',
                        params: { id: relayId, on: !!value },
                    }),
            };
        }

        const lightSwitchMatch = /^(Light|RGB|RGBW|CCT)(\d+)\.Switch$/.exec(stateId);
        if (lightSwitchMatch && this.getDeviceGen() >= 2) {
            const component = lightSwitchMatch[1];
            const id = Number(lightSwitchMatch[2]);
            return {
                mqtt_cmd: '<mqttprefix>/rpc',
                http_cmd_funct: value =>
                    JSON.stringify({
        ...
        const coverActionMatch = /^Cover(\d+)\.(Open|Close|Stop)$/.exec(stateId);
        if (coverActionMatch && this.getDeviceGen() >= 2) {
            const id = Number(coverActionMatch[1]);
            const action = coverActionMatch[2];
            return {
                mqtt_cmd: '<mqttprefix>/rpc',
                http_cmd_funct: () =>
                    JSON.stringify({
        ...
        const coverPositionMatch = /^Cover(\d+)\.(Position|TargetPosition)$/.exec(stateId);
        if (coverPositionMatch && this.getDeviceGen() >= 2) {
            const id = Number(coverPositionMatch[1]);
            return {
                mqtt_cmd: '<mqttprefix>/rpc',
                http_cmd_funct: value =>
                    JSON.stringify({
    ...
    async executeHttpControlCommand(httpState, publishValue, fullStateId) {
        if (this.getDeviceGen() === 1) {
            return await super.executeHttpControlCommand(httpState, publishValue, fullStateId);
        }

        if (this.getDeviceGen() >= 2 && httpState?.mqtt_cmd) {
            this.adapter.log.debug(
                sanitizeLogMessage(
                    `[HTTP] State command ${fullStateId} routed to RPC for ${this.getLogInfo()}: ${JSON.stringify(publishValue)}`,
                ),
            );
            await this.publishStateValue(httpState.mqtt_cmd, publishValue);
            return undefined;
        }
    ...
    async afterHttpControlCommand(fullStateId) {
        this.adapter.log.debug(`[HTTP] Refreshing ${this.getLogInfo()} after command on ${fullStateId}`);
        if (this.rawOnly || !this.http || Object.keys(this.http).length === 0) {
    ...
    async createRawObjects() {
        ...
        await new Promise(resolve => this.objectHelper.processObjectQueue(() => resolve()));
    }
    ...
    async pollRawPayloads() {
        ...
        for (const endpoint of endpoints) {
            try {
                ...
                if (this.isHttpRawResponseDebugEnabled()) {
                    this.adapter.log.debug(
                        `[HTTP Raw Debug] ${this.getLogInfo()} ${endpoint.url}: ${truncateLogMessage(body)}`,
                    );
                }
                const stateId =
    ...
    async pollGenericCapabilities() {
        ...
        try {
            ...
            const debugMissing = [];
            const debugWrites = [];
            const debugPaths = this.isHttpPollingDebugEnabled()
                ? this.getHttpDebugPaths({
                      status: this.capability.status,
                      config: this.capability.config,
                      info: this.capability.info,
                  })
                : [];
            const statesById = new Map(this.capability.states.map(state => [state.id, state]));
            const trackedStates = new Map([
                ...Object.entries(this.genericStates || {}),
                ...this.capability.states.map(state => [state.id, state]),
            ]);

            for (const [trackedStateId, trackedState] of trackedStates) {
                const state = statesById.get(trackedStateId) || trackedState;
            ...
            this.logHttpPollingExtraction('generic capabilities', debugExtracted, debugMissing, debugWrites);
        } catch (err) {
            this.adapter.log.debug(
                sanitizeLogMessage(
    ...
    async start() {
        try {
            this.adapter.log.debug(
                `[HTTP] Starting device ip=${this.getIP()}, configuredName=${this.customName || '<none>'}, activeProtocol=${this.adapter.config.protocol || '<unset>'}`,
            );
            await this.discover();
            this.adapter.log.debug(
                `[HTTP] Device loaded id=${this.getDeviceId()}, type=${this.getDeviceType()}, class=${this.getDeviceClass()}, gen=${this.getDeviceGen()}, profile=${this.type}`,
            );
            ...
            if (!this.rawOnly) {
                ...
                this.adapter.log.debug(`[HTTP] createObjects for device ${this.getDeviceId()}`);
                await this.createObjects();
                await this.setIP(this.getIP(), 'HTTP polling');
                if (this.adapter.config.httpSaveRawJson) {
                    await this.createRawObjects();
                ...
                this.adapter.log.debug(`[HTTP] createGenericCapabilityObjects for device ${this.getDeviceId()}`);
...
class HTTPPollingServer extends BaseServer {
    ...
    isHttpDiscoveryDebugEnabled() {
        return this.adapter?.config?.httpDebugDiscovery === true;
    }

    logHttpAuth({ ip, scheme }) {
        if (this.isHttpDiscoveryDebugEnabled()) {
            this.adapter.log.debug(`[HTTP] Discovery auth scheme detected for ${ip}: scheme=${scheme}`);
        }
    }
    ...
    logHttpAuthFailed({ scheme, endpoint, status }) {
        if (this.isHttpDiscoveryDebugEnabled()) {
            this.adapter.log.debug(
                `[HTTP] Discovery auth failed: endpoint=${sanitizeLogMessage(endpoint)}, scheme=${scheme || '<unknown>'}, status=${status ?? '<none>'}`,
            );
        }
    }

    async performProbeRequest(ip, url, auth) {
        return await performHttpRequest(ip, url, this.getTimeout(), auth, this);
    }
    ...
    async probeGetWithRetry(ip, url) {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                lastError = err;
                if (attempt < retries && this.isHttpDiscoveryDebugEnabled()) {
                    this.adapter.log.debug(
                        sanitizeLogMessage(
    ...
    async probeIp(ip) {
        try {
            ...
            return {
                ...
                type: normalized.deviceType,
                gen: normalized.gen,
                generation: normalized.gen,
                mac: normalized.serialId,
    ...
    async scanNetworkRanges(options = {}) {
        ...
        const results = await runWithConcurrency(ips, this.getMaxParallel(), async ip => {
            const device = await this.probeIp(ip);
            if (device && typeof options.onDevice === 'function') {
                await options.onDevice(device);
            }
            return device;
        });
        const devices = results.filter(Boolean);
        const failed = ips.length - devices.length;
        this.adapter.log.info(
            `[HTTP] Discovery summary: scanned=${ips.length}, found=${devices.length}, failed=${failed}`,
        );
        if (this.isHttpDiscoveryDebugEnabled()) {
            devices.forEach(device =>
                this.adapter.log.debug(
                    `[HTTP] Discovered device ip=${device.ip}, id=${device.deviceId}, capabilities=${device.capabilityCount}`,
                ),
            );
        }
        return devices;
    ...
    async getRegistryDevices() {
        if (typeof this.adapter.getDevicesAsync !== 'function' || typeof this.adapter.getStateAsync !== 'function') {
            this.adapter.log.debug('[HTTP] ioBroker device registry is not available for HTTP polling startup');
            return [];
        }
        ...
        for (const device of devices) {
            const shortDeviceId = this.adapter.removeNamespace
                ? this.adapter.removeNamespace(device._id)
                : String(device._id || '').replace(`${this.adapter.namespace}.`, '');

            if (!shortDeviceId || shortDeviceId === device._id) {
                continue;
            }

            const hostnameState = await this.adapter.getStateAsync(`${shortDeviceId}.hostname`);
            const networkIpState = await this.adapter.getStateAsync(`${shortDeviceId}.Network.ip`);
            const ip = hostnameState?.val || networkIpState?.val;
            if (!ip) {
                continue;
            }

            const classState = await this.adapter.getStateAsync(`${shortDeviceId}.class`);
            const typeState = await this.adapter.getStateAsync(`${shortDeviceId}.type`);
            const protocolState = await this.adapter.getStateAsync(`${shortDeviceId}.protocol`);
            registryDevices.push({
                ip: String(ip),
                hostname: String(ip),
                deviceId: shortDeviceId,
                id: shortDeviceId,
                name: typeof device.common?.name === 'string' ? device.common.name : shortDeviceId,
                model: typeState?.val || classState?.val,
                type: typeState?.val || classState?.val,
                source: 'ioBroker-registry',
                protocol: protocolState?.val,
                enabled: true,
            });
        }
    ...
    hasClientForDevice(device) {
        return !!(
            this.clients[device.ip] ||
            Object.values(this.clients).some(
                client =>
                    device.deviceId &&
                    typeof client.getDeviceId === 'function' &&
                    client.getDeviceId() === device.deviceId,
            )
        );
    }

    startDeviceClient(device) {
        if (!device?.ip || this.hasClientForDevice(device)) {
            return undefined;
        }
        const client = new HTTPPollingClient(this.adapter, this.objectHelper, this.eventEmitter, device);
        this.clients[device.ip] = client;
        this.adapter.log.debug(
            `[HTTP] Creating HTTPPollingClient for ${device.deviceId || device.ip}; ip=${device.ip}, source=${device.source || '<unknown>'}`,
        );
        client.startPromise = client.start().then(() => {
            this.adapter.log.debug(
                `[HTTP] HTTPPollingClient initialized for ${client.getDeviceId?.() || device.deviceId || device.ip}`,
            );
            return client;
        });
        return client;
    ...
    async waitForClientStarts(clients) {
        const startPromises = clients
            .filter(client => client?.startPromise)
            .map(async client => {
                try {
                    ...
                    this.adapter.log.error(
                        sanitizeLogMessage(
                            `[HTTP] HTTPPollingClient startup failed for ${client.getLogInfo?.() || '<unknown>'}: ${err.message || err}`,
                        ),
                    );
                }
            });
    ...
    async listen() {
        this.adapter.log.info(`[HTTP] HTTPPollingServer listen start; active protocol=${this.adapter.config.protocol}`);
        const configuredDevices = this.getConfiguredDevices();
        const registryDevices = await this.getRegistryDevices();
        const knownDevices = mergeHttpDevices(configuredDevices, registryDevices, true);
        this.adapter.log.info(
            `[HTTP] Loaded configured/registry HTTP device(s): total=${knownDevices.length}, configured=${configuredDevices.length}, registry=${registryDevices.length}`,
        );
        this.adapter.log.debug(
            `[HTTP] Startup registry devices: configured=${configuredDevices.map(device => device.deviceId || device.ip).join(', ') || '<none>'}; registry=${registryDevices.map(device => device.deviceId || device.ip).join(', ') || '<none>'}`,
        );
        configuredDevices.forEach(device =>
            this.adapter.log.debug(
                `[HTTP] Configured device ip=${device.ip}, id=${device.deviceId || '<unknown>'}, hasCustomName=${!!device.name}, authMode=${device.authMode || '<default>'}`,
            ),
        );
        registryDevices.forEach(device =>
            this.adapter.log.debug(
                `[HTTP] Registry device ip=${device.ip}, id=${device.deviceId || '<unknown>'}, protocol=${device.protocol || '<unknown>'}, source=${device.source}`,
            ),
        );
        const autoCreate = this.adapter.config.httpAutoCreate !== false;
        await this.waitForClientStarts(knownDevices.map(device => this.startDeviceClient(device)));
        if (!this.adapter.config.httpDiscoveryEnabled) {
            this.adapter.log.info('[HTTP] Skipping network discovery because discovery=false');
        }
        const discoveredDevices = await this.scanNetworkRanges({
            onDevice: autoCreate
                ? async device => {
                      const client = this.startDeviceClient(device);
                      await this.waitForClientStarts([client]);
                  }
                : undefined,
        });
        ...
        const devices = mergeHttpDevices(knownDevices, discoveredDevices, autoCreate);
        this.adapter.log.info(`[HTTP] HTTP polling clients ready: ${Object.keys(this.clients).length}`);

        if (devices.length === 0) {
            this.adapter.log.info(
                '[HTTP] No HTTP polling devices configured, discovered, or found in the ioBroker registry',
            );
            return;
        }

        await this.waitForClientStarts(devices.map(device => this.startDeviceClient(device)));
    }
