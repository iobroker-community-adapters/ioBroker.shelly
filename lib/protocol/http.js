    ...
    async start() {
        try {
            ...
            if (!this.rawOnly) {
                ...
                this.adapter.log.info(`[HTTP] createObjects for device ${this.getDeviceId()}`);
                ...
                this.adapter.log.info(`[HTTP] createGenericCapabilityObjects for device ${this.getDeviceId()}`);
...
class HTTPPollingServer extends BaseServer {
    ...
    async scanNetworkRanges(options = {}) {
        ...
        this.adapter.log.info(`[HTTP] Discovery found ${devices.length} device(s)`);
    ...
    async listen() {
        ...
        this.adapter.log.info(
            `[HTTP] Loaded ${knownDevices.length} configured/registry HTTP device(s) (configured=${configuredDevices.length}, registry=${registryDevices.length})`,
        );
        this.adapter.log.debug(
        ...
        this.adapter.log.info(`[HTTP] Starting ${devices.length} HTTP polling device client(s)`);
