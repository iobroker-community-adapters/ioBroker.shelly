...
function sanitizePersistedHttpDevice(device) {
    const persisted = {};
    for (const key of [
        'deviceId',
        'id',
        ...
        if (device?.[key] !== undefined && device[key] !== null && device[key] !== '') {
            persisted[key] = device[key];
        }
    }
    persisted.deviceId ||= persisted.id;
    persisted.id ||= persisted.deviceId;
    persisted.source = 'http-discovery';
    persisted.enabled = true;
    return persisted;
}

function mergePersistedHttpDevices(existingDevices = [], newDevices = []) {
    const devicesByKey = new Map();
    for (const device of [...existingDevices, ...newDevices]) {
        if (!device?.ip || device.enabled === false) {
            continue;
        }
        const sanitized = sanitizePersistedHttpDevice(device);
        devicesByKey.set(getHttpDeviceMergeKey(sanitized), sanitized);
    }
    return Array.from(devicesByKey.values());
}
...
class HTTPPollingServer extends BaseServer {
    ...
    getPersistedDevices() {
        return Array.isArray(this.adapter.config.httpDiscoveredDevices)
            ? this.adapter.config.httpDiscoveredDevices
                  .filter(device => device && device.ip && device.enabled !== false)
                  .map(device => ({
                      ...sanitizePersistedHttpDevice(device),
                      source: 'http-discovery',
                  }))
            : [];
    }

    async persistDiscoveredDevices(devices) {
        if (!Array.isArray(devices) || !devices.length) {
            return;
        }

        const currentDevices = Array.isArray(this.adapter.config.httpDiscoveredDevices)
            ? this.adapter.config.httpDiscoveredDevices
            : [];
        const mergedDevices = mergePersistedHttpDevices(currentDevices, devices);
        this.adapter.config.httpDiscoveredDevices = mergedDevices;

        if (typeof this.adapter.extendForeignObjectAsync !== 'function') {
            this.adapter.log.warn('[HTTP] Unable to persist discovered HTTP devices: adapter persistence API missing');
            return;
        }

        try {
            await this.adapter.extendForeignObjectAsync(`system.adapter.${this.adapter.namespace}`, {
                native: {
                    ...this.adapter.config,
                    httpDiscoveredDevices: mergedDevices,
                },
            });
            this.adapter.log.info(
                `[HTTP] Persisted ${mergedDevices.length} discovered HTTP polling device(s) in native.httpDiscoveredDevices`,
            );
        } catch (err) {
            this.adapter.log.warn(
                sanitizeLogMessage(`[HTTP] Unable to persist discovered HTTP devices: ${err.message || err}`),
            );
        }
    }
    ...
    startDeviceClient(device) {
        ...
        this.adapter.log.info(
            `[HTTP] Starting client for ip=${device.ip}, configuredId=${device.deviceId || '<unknown>'}, source=${device.source || '<unknown>'}`,
        );
        void client.start();
        return client;
    }
    ...
    async listen() {
        ...
        const persistedDevices = this.getPersistedDevices();
        const knownDevices = mergeHttpDevices(configuredDevices, persistedDevices, true);
        this.adapter.log.info(
            `[HTTP] Loaded ${knownDevices.length} configured/persisted HTTP device(s) (configured=${configuredDevices.length}, persisted=${persistedDevices.length})`,
        );
        ...
        persistedDevices.forEach(device =>
            this.adapter.log.debug(
                `[HTTP] Persisted device ip=${device.ip}, id=${device.deviceId || '<unknown>'}, gen=${device.gen || '<unknown>'}, source=${device.source || '<unknown>'}`,
            ),
        );
        const autoCreate = this.adapter.config.httpAutoCreate !== false;
        knownDevices.forEach(device => this.startDeviceClient(device));
        if (!this.adapter.config.httpDiscoveryEnabled) {
        ...
        const discoveredDevices = await this.scanNetworkRanges({
            onDevice: autoCreate
                ? async device => {
                      await this.persistDiscoveredDevices([device]);
                      this.startDeviceClient(device);
                  }
                : undefined,
        });
        if (autoCreate) {
            await this.persistDiscoveredDevices(discoveredDevices);
        }
        ...
        devices.forEach(device => this.startDeviceClient(device));
    }
