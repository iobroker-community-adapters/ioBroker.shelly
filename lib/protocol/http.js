...
class HTTPPollingClient extends BaseClient {
    ...
    async markHttpFailure(url, err) {
        this.failureCount++;
        this.adapter.log.debug(
            `[HTTP] Request failed for ${this.getLogInfo()} ${url}; failure ${this.failureCount}/${OFFLINE_FAILURE_THRESHOLD}: ${err}`,
        );
    ...
    async readStatusAndConfig() {
        if (this.getDeviceGen() === 1) {
            ...
            this.rawPayloads.status = JSON.stringify(status);
            this.rawPayloads.settings = JSON.stringify(settings);
            ...
            const config = this.rawPayloads.config
                ...
                : parseJson(await this.requestWithRetry('/rpc/Sys.GetConfig'), {});
            this.rawPayloads.status = JSON.stringify(status);
            this.rawPayloads.config = JSON.stringify(config);
            this.capability = analyzeShellyCapabilities({
    ...
    async pollGenericCapabilities() {
        ...
        try {
            ...
            this.adapter.log.debug(`[HTTP] Generic capability poll failed for ${this.getLogInfo()}: ${err}`);
        } finally {
            this.scheduleGenericPolling();
        }
    }
...
class HTTPPollingServer extends BaseServer {
    ...
    async probeGetWithRetry(ip, url) {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                lastError = err;
                if (attempt < retries) {
                    this.adapter.log.debug(`[HTTP] Discovery retry ${attempt + 1}/${retries} for ${ip} ${url}: ${err}`);
                }
            }
        }
        throw lastError;
    ...
    async probeAnalysis(ip, normalized, info) {
        try {
            if (normalized.gen >= 2) {
                const [statusBody, configBody] = await Promise.all([
                    this.probeGetWithRetry(ip, '/rpc/Shelly.GetStatus'),
                    this.probeGetWithRetry(ip, '/rpc/Sys.GetConfig'),
                ]);
                return analyzeShellyCapabilities({
                    gen: normalized.gen,
                    status: parseJson(statusBody, {}),
                    config: parseJson(configBody, {}),
                    info,
                });
            }
            ...
            this.adapter.log.debug(`[HTTP] Capability probe failed for ${ip}: ${err.message || err}`);
            return undefined;
        }
    }
