...
class MQTTClient {
  ...
  sendState2Client(topic, state, qos, dup, retain, cb) {
    ...
    if (qos > 0) {
      this.qos[this.messageId] = {
        ts: Date.now(),
        cmd: 'pubrec',
        count: 0,
        message: { topic: topic, payload: state, qos: qos, dup: true, retain: retain, messageId: this.messageId }
      };
      this.resendState2Client(this.messageId);
    }
  }

  resendState2Client(messageId) {
    let retaintime = 10 * 1000;
    if (this.qos[messageId]) {
      this.qos[messageId].resendid = setTimeout(() => {
        ...
        this.resendState2Client(messageId);
      }, retaintime);
    }
  }

  deleteResend2Client(messageId) {
    if (this.qos[messageId]) {
          ...
          if (state && state.state) {
            ...
            try {
              ...
              if (body && body === '401 Unauthorized') {
                ...
                this.adapter.log.error('Error ' + error + ' in function state.mqtt.http_publish_funct for state ' + stateid + ' for ' + this.getName());
              }
            }
          }
        }
  ...
  listener() {
    ...
    this.client.on('connect', (packet) => {
      ...
      if (this.deviceExist()) {
        ...
        this.adapter.log.error('Configuratin for Shelly device ' + this.getName() + ' for MQTT does not exist!');
        this.client.connack({ returnCode: 4 });
      }
    });
    ...
    this.client.on('puback', (packet) => {
      // remove this message from queue
      this.deleteResend2Client(47111);
    });
