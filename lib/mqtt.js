...
class MQTTClient {
   ...
   */
  getDeviceName() {
    let devicename = this.getDeviceType() + '#' + this.getSerialId() + '#1';
    return devicename;
  }
  ...
  getDeviceType() {
    ...
    let devicetype = datapoints.getDeviceNameForMQTT(deviceid);
    return devicetype;
  }
  ...
  createObjects() {
    if (Object.keys(this.device).length === 0) {
      let deviceid = this.getDeviceId();
      let devices = datapoints.getDeviceByType(deviceid);
      if (devices) {
