...
class MQTTClient {
  ...
  async createIoBrokerState(topic, payload) {
    ...
    for (let i in dps) {
      ...
      try {
        value = dp.mqtt && dp.mqtt.mqtt_publish_funct && isAsync(dp.mqtt.mqtt_publish_funct) ? await dp.mqtt.mqtt_publish_funct(value, this) : dp.mqtt.mqtt_publish_funct(value, this);
    ...
    for (let i in this.http) {
      ...
      try {
        ...
        for (let j in states) {
          ...
          try {
            let value = state.mqtt && state.mqtt.http_publish_funct && isAsync(state.mqtt.http_publish_funct) ? await state.mqtt.http_publish_funct(body, this) : state.mqtt.http_publish_funct(body, this);
            if (!this.states.hasOwnProperty(stateid) || this.states[stateid] !== value) {
              this.states[stateid] = value;
              this.objectHelper.setOrUpdateObject(stateid, {
                type: 'state',
