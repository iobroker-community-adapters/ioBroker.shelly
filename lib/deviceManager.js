...
class ShellyDeviceManagement extends dm_utils_1.DeviceManagement {
    ...
    async handleDiscoverDevices(context) {
        ...
        try {
            ...
            if (!newDevices.length && existingDevices.length) {
                ...
                const result = await context.showForm({ type: 'panel', items }, {
                    data,
    ...
    async provisionDevices(devices, context) {
            ...
            try {
                ...
                if (err instanceof HttpAuthError) {
                    ...
                    if (httpPass) {
                        ...
                        try {
                            await this.provisionSingleDevice(dev, mqttServer, mqttUser, mqttPass, httpPass, timeZone, httpPass);
                            results.push(`✓ ${dev.name}${dev.customName.trim() ? ` → ${dev.customName.trim()}` : ''}`);
                    ...
                    if (formResult?.devicePassword) {
                        ...
                        try {
                            await this.provisionSingleDevice(dev, mqttServer, mqttUser, mqttPass, httpPass, timeZone, devPass);
                            results.push(`✓ ${dev.name}${dev.customName.trim() ? ` → ${dev.customName.trim()}` : ''}`);
        ...
        await context.showForm({ type: 'panel', items }, { title: adapter_core_1.I18n.getTranslatedObject('Configuration results'), buttons: ['close'] });
    }
    async provisionSingleDevice(dev, mqttServer, mqttUser, mqttPass, httpPass, timeZone, devicePassword) {
