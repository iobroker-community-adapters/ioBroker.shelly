...
async function shuterDuration(self, newval) {
  try {
    let id = self.getDeviceName() + '.Shutter.Duration';
...
let shellyswitch = {
  ...
  'Shutter.Close': {
    coap: {
      ...
      http_cmd: '/roller/0',
      http_cmd_funct: async (value, self) => { return { go: 'close', duration: await shuterDuration(self, 0) }; },
      // http_publish: '/settings',
    ...
    mqtt: {
      ...
      // http_publish_funct: (value) => { return JSON.parse(value) ? JSON.parse(value).rollers.maxtime : undefined; },
      // http_cmd: '/roller/0',
      // http_cmd_funct: async (value,self) => { return { go: 'close', duration: await shuterDuration(self) }
    },
    common: {
      'name': 'Close',
      'type': 'boolean',
  ...
  'Shutter.Open': {
    coap: {
      ...
      http_cmd: '/roller/0',
      http_cmd_funct: async (value, self) => { return { go: 'open', duration: await shuterDuration(self, 0) }; },
    },
    mqtt: {
      mqtt_publish: 'shellies/shellyswitch-<deviceid>/roller/0',
  ...
  'Shutter.Pause': {
    coap: {
      ...
      http_cmd: '/roller/0',
      http_cmd_funct: async (value, self) => { return { go: 'stop', duration: await shuterDuration(self, 0) }; }
    },
    mqtt: {
      mqtt_publish: 'shellies/shellyswitch-<deviceid>/roller/0',
      mqtt_cmd: 'shellies/shellyswitch-<deviceid>/roller/0/command',
  ...
  'Shutter.Duration': {
    coap: {
      http_publish: '/settings',
      http_publish_funct: async (value, self) => { return await shuterDuration(self); }
  ...
  'Shutter.Position': {
    coap: {
      ...
      http_cmd: '/roller/0',
      http_cmd_funct: async (value, self) => { return { 'go': 'to_pos', 'roller_pos': value, 'duration': await shuterDuration(self, 0) }; }
    },
    mqtt: {
