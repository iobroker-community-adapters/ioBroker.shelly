...
let shellyplug = {
  ...
  'Relay0.Power': {
    coap: {
      ...
      coap_publish_funct: (value) => { return value ? (Math.round(JSON.parse(value).meters[0].power * 100) / 100) : undefined; }
    },
    mqtt: {
  ...
  'Relay0.Energy': {
    coap: {
      ...
      coap_publish_funct: (value) => { return value ? (Math.round(JSON.parse((value).meters[0].power / 60) * 100) / 100) : undefined; }
    },
    mqtt: {
  ...
  'Relay0.overpowerValue': {
    ...
    mqtt: {
      mqtt_publish: 'shellies/shellyplug-s-<deviceid>/relay/0/overpower_value',
    },
    common: {
