...
let shellydw2 = {
  ...
  'sensor.temperature': {
    coap: {
      coap_publish: ['3101', '3102'], // CoAP >= FW 1.8
      coap_publish_funct: async (value, self) => { return await shellyHelper.getValue(self, 'sensor.temperatureunit') == 'F' ? value[1] : value[0]; }
    },
    mqtt: {
  ...
  'sensor.temperatureunit': {
    coap: {
      ...
      http_publish_funct: (value) => { return value ? JSON.parse(value).sensors.temperature_unit : undefined; },
      http_cmd: '/settings',
      http_cmd_funct: (value) => { return { temperature_units: value }; }
    },
    mqtt: {
      http_publish: '/settings',
      http_publish_funct: (value) => { return value ? JSON.parse(value).sensors.temperature_unit : undefined; },
  ...
  'sensor.wakeupevent': {
    ...
    mqtt: {
      mqtt_publish: 'shellies/shellydw2-<deviceid>/sensor/wakeupevent',
    },
    common: {
      'name': 'Wakeup Event',
      'type': 'string',
