    ...
    'Mqtt.topicPrefix': {
        ...
        mqtt: {
            ...
            http_publish_funct: (value, self) => {
                ...
                const mqttPrefix = self.getMqttPrefix();
                if (
                    result &&
                    mqttPrefix &&
                    result !== mqttPrefix &&
                    !result.endsWith(`/${mqttPrefix}`) &&
                    !mqttPrefix.endsWith(`/${result}`)
                ) {
                    self.adapter.log.warn(
                        `[Mqtt.topicPrefix] ${self.getLogInfo()}: Configured mqtt topic prefix "${result}" and actual topic prefix "${mqttPrefix}" do not match. Please check configuration`,
                    );
                }
