...
const defaultsgen1 = {
    ...
    'Mqtt.topicPrefix': {
        ...
        mqtt: {
            http_publish: '/rpc/settings',
            http_publish_funct: (value, self) => {
                const result = value ? JSON.parse(value)?.mqtt?.id : undefined;
                if (result !== self.getMqttPrefix()) {
                    self.adapter.log.warn(`[Mqtt.topicPrefix] ${self.getName()}: Configured mqtt topic prefix and actual topic prefix do not match. Please check configuration`);
                }
    ...
    'Mqtt.rpcNotifications': {
        mqtt: {
            ...
            http_publish_funct: (value, self) => {
                ...
                if (result === false) {
                    self.adapter.log.warn(`[Mqtt.rpcNotifications] ${self.getName()}: "RPC Status Notifications" are disabled. Enable to get all information.`);
                }
    ...
    'Mqtt.statusNotifications': {
        mqtt: {
            ...
            http_publish_funct: (value, self) => {
                ...
                if (result === false) {
                    self.adapter.log.warn(`[Mqtt.statusNotifications] ${self.getName()}: "General Status Notifications" are disabled. Enable to get all information.`);
                }
