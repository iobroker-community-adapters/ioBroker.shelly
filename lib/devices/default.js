...
function areEquivalentTopicPrefixes(configuredPrefix, actualPrefix) {
    const configured = normalizeTopicPrefix(configuredPrefix);
    const actual = normalizeTopicPrefix(actualPrefix);
    ...
    const configuredWithoutShelly = configured.startsWith('shelly/') ? configured.substring(7) : configured;
    const actualWithoutShelly = actual.startsWith('shelly/') ? actual.substring(7) : actual;

    return configuredWithoutShelly === actualWithoutShelly;
}
    ...
    'Mqtt.topicPrefix': {
        ...
        mqtt: {
            ...
            http_publish_funct: (value, self) => {
                ...
                if (result && self.getMqttPrefix() && !areEquivalentTopicPrefixes(result, self.getMqttPrefix())) {
...
const defaultsgen2 = {
    ...
    firmwareupdateProgress: {
        ...
        common: {
            name: {
                en: 'Firmware update progress',
                de: 'Firmware Update Fortschritt',
                ru: 'Прогресс обновления Firmware',
                pt: 'Progresso de atualização de firmware',
                nl: 'Voortgang firmware-update',
                fr: 'Mise à jour du firmware',
                it: 'Aggiornamento firmware',
                es: 'Progreso de actualización de firmware',
                pl: 'Postępy aktualizacji oprogramowania firmowego',
                uk: 'Прогрес оновлення прошивки',
                'zh-cn': '固件更新进度',
            },
            type: 'number',
            role: 'value',
    ...
    uptime: {
        mqtt: {
            mqtt_publish: '<mqttprefix>/status/sys',
            mqtt_publish_funct: value => (value ? parseInt(JSON.parse(value).uptime) : undefined),
        },
        common: {
            name: {
                en: 'Device running since',
    ...
    rssi: {
        mqtt: {
            ...
            mqtt_publish_funct: value => (value ? JSON.parse(value).rssi : undefined),
        },
        common: {
            name: {
                en: 'Received Signal Strength Indication',
