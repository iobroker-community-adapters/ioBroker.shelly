...
const defaultsgen2 = {
    ...
    'WiFi.apClients': {
        mqtt: {
            http_publish: `/rpc/Wifi.ListApClients`,
            http_publish_funct: (value, self) => {
                const rawClients = JSON.parse(value)?.ap_clients ?? [];
                const clientEntries = Object.assign({}, ...rawClients.map(entry => ({ [entry.ip]: entry.mport })));
                self.adapter.updateApEntry(self.getIP(), clientEntries);
                return JSON.stringify(rawClients);
            },
        },
        common: {
            name: {
                en: 'Access point clients',
                de: 'Access Point Geräte',
                ru: 'Клиенты точки доступа',
                pt: 'Clientes do ponto de acesso',
                nl: 'Toegangspuntclients',
                fr: "Clients du point d'accès",
                it: 'Client del punto di accesso',
                es: 'Clientes del punto de acceso',
                pl: 'Klienci punktu dostępu',
                uk: 'Клієнти точки доступу',
                'zh-cn': '接入点客户端',
            },
            type: 'string',
            role: 'json',
