...
let shellydimmer = {
  'light.Switch': {
    coap: {
      ...
      coap_publish_funct: (value) => { return getCoapValue(121, value.G) == 1 ? true : false; },
      http_cmd: '/light/0',
  ...
  'light.brightness': {
    coap: {
      coap_publish_funct: (value) => { return getCoapValue(111, value.G); },
      http_cmd: '/light/0',
...
let autooff = false;
schedule("* * * * *", function () {
  if (compareTime('02:43', '03:23', "between", null)) {
    setState("sonoff.0.Hausbeleuchtung.POWER"/*Hausbeleuchtung POWER*/, true);
  }
  lux = getState("sonoff.0.Hausbeleuchtung.BH1750_Illuminance").val;
  luxHell = 0;
  if (compareTime("06:10", "08:00", "between") || compareTime("16:00", "23:15", "between")) {
    if (lux == luxHell) {
      setState("sonoff.0.Hausbeleuchtung.POWER"/*Hausbeleuchtung POWER*/, true);
      autooff = false;
      console.log('Licht ist an da Zeit zwischen 5:30 und 8:00 oder 16:00 und 23:30');
    } else {
      if (autooff == false) {
        setState("sonoff.0.Hausbeleuchtung.POWER"/*Hausbeleuchtung POWER*/, false);
        autooff = true;
        console.log('Licht ist aus da Zeit zwischen 5:30 und 8:00 oder 16:00 und 23:30');
      }
    }
  } else {
    if (autooff == false) {
      setState("sonoff.0.Hausbeleuchtung.POWER"/*Hausbeleuchtung POWER*/, false);
      autooff = true;
      console.log('Licht ist aus da Zeit nicht zwischen 5:30 und 8:00 oder 16:00 und 23:30');
    }
  }
});