...
let shellydimmer2 = {
  ...
  'lights.dim': {
    coap: {
      http_publish: '/light/0',
      http_publish_funct: (value) => { return value ? JSON.parse(value).dim : undefined; },
      http_cmd: '/light/0',
      http_cmd_funct: (value) => { return { dim: value }; }
    },
    mqtt: {
      mqtt_publish: 'shellies/<mqttprefix>/light/0/status',
      mqtt_publish_funct: (value) => { return value ? JSON.parse(value).dim : undefined; },
      mqtt_cmd: 'shellies/<mqttprefix>/light/0/set',
      mqtt_cmd_funct: (value) => { return value ? { dim: value } : undefined; }
    },
    common: {
      'name': 'Dim',
      'type': 'string',
      ...
      'read': true,
      'write': true,
      'states': 'up:up;down:down;stop:stop;cycle:cycle'
    }
  }
};
