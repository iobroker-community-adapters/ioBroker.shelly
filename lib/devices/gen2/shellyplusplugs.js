...
const shellyplusplugs = {
    ...
    'PLUGS_UI.SwitchOnColor': {
        mqtt: {
            http_publish: '/rpc/PLUGS_UI.GetConfig',
            http_publish_funct: value => value ? JSON.parse(value)?.leds?.colors?.['switch:0']?.on?.rgb.join(',') : undefined,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
    ...
    'PLUGS_UI.SwitchOffBrightness': {
        mqtt: {
            http_publish: '/rpc/PLUGS_UI.GetConfig',
            http_publish_funct: value => value ? JSON.parse(value)?.leds?.colors?.['switch:0']?.off?.brightness : undefined,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
    ...
    'PLUGS_UI.SwitchOffColor': {
        mqtt: {
            http_publish: '/rpc/PLUGS_UI.GetConfig',
            http_publish_funct: value => value ? JSON.parse(value)?.leds?.colors?.['switch:0']?.off?.rgb.join(',') : undefined,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
