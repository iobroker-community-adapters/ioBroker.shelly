...
let shellyem3 = {
  ...
  'Emeter1.Power': {
    coap: {
      // coap_publish_funct: (value) => {  return (Math.round(value.G[2][2] * 100) / 100); }
      coap_publish_funct: (value) => { return (Math.round(shellyHelper.getCoapValue(121, value.G) * 100) / 100); }
    },
    mqtt: {
  ...
  'Emeter2.Power': {
    coap: {
      // coap_publish_funct: (value) => {  return (Math.round(value.G[2][2] * 100) / 100); }
      coap_publish_funct: (value) => { return (Math.round(shellyHelper.getCoapValue(131, value.G) * 100) / 100); }
    },
    mqtt: {
