 ...
 */
function addLight(deviceObj, lightId) {

    deviceObj[`Light${lightId}.ChannelName`] = {
        mqtt: {
            http_publish: `/rpc/Light.GetConfig?id=${lightId}`,
            http_publish_funct: async (value, self) => {
                return value ? await shellyHelper.setChannelName(self, `Light${lightId}`, JSON.parse(value).name) : undefined;
            },
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
 ...
 * - Color (RGB color in HEX format)
 * - Effect (predefined light effects)
 */
function addRGB(deviceObj, rgbId) {
    ...
    deviceObj[`RGB${rgbId}.Color`] = {
        mqtt: {
            mqtt_publish: `<mqttprefix>/status/rgb:${rgbId}`,
            mqtt_publish_funct: value => JSON.parse(value).color,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
    ...
    deviceObj[`RGB${rgbId}.Effect`] = {
        mqtt: {
            mqtt_publish: `<mqttprefix>/status/rgb:${rgbId}`,
            mqtt_publish_funct: value => JSON.parse(value).effect,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
 ...
 * - Brightness (brightness level from 0 to 100)
 * - Color (RGB color in HEX format)
 * - White (white channel intensity level from 0 to 100)
 * - Effect (predefined light effects)
 */
