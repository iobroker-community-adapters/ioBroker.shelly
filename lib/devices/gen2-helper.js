...
function addCover(deviceObj, coverId, hasSlat) {
    ...
    deviceObj[`Cover${coverId}.FavoritePos1.GoTo`] = {
        mqtt: {
            ...
            mqtt_cmd_funct: async (_value, self) => {
                const pos = await shellyHelper.getState(self, 'Favorites.Cover${coverId}.FavoritePos1.Pos');
    ...
    deviceObj[`Cover${coverId}.FavoritePos2.GoTo`] = {
        mqtt: {
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: async (_value, self) => {
                const pos = await shellyHelper.getState(self, 'Favorites.Cover${coverId}.FavoritePos2.Pos');
                return (pos === undefined) ? undefined : JSON.stringify({
                    id: self.getNextMsgId(),
                    src: 'iobroker',
                    method: 'Cover.GoToPosition',
    ...
    deviceObj[`Cover${coverId}.FavoritePos3.GoTo`] = {
        mqtt: {
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: async (_value, self) => {
                const pos = await shellyHelper.getState(self, 'Favorites.Cover${coverId}.FavoritePos3.Pos');
                return (pos === undefined) ? undefined : JSON.stringify({
                    id: self.getNextMsgId(),
                    src: 'iobroker',
                    method: 'Cover.GoToPosition',
