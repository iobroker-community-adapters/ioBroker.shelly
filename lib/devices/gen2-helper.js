...
function addRGB(deviceObj, rgbId) {
    ...
    deviceObj[`RGB${rgbId}.ColorRGB`] = {
        mqtt: {
            mqtt_publish: `<mqttprefix>/status/rgb:${rgbId}`,
            mqtt_publish_funct: value => Array.isArray(JSON.parse(value).rgb) ? JSON.parse(value).rgb.join(',') : null,
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
    ...
    deviceObj[`RGB${rgbId}.Color`] = {
        mqtt: {
            mqtt_publish: `<mqttprefix>/status/rgb:${rgbId}`,
            mqtt_publish_funct: value => '#' + JSON.parse(value).rgb.map(x => x.toString(16).padStart(2, '0')).join(''),
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
                    id: self.getNextMsgId(),
    ...
    deviceObj[`RGB${rgbId}.ChannelName`] = {
        mqtt: {
            ...
            http_publish_funct: async (value, self) => {
                return value ? await shellyHelper.setChannelName(self, `RGB${rgbId}`, JSON.parse(value).name) : undefined;
            },
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
...
function addRGBW(deviceObj, rgbwId) {
    ...
    deviceObj[`RGBW${rgbwId}.ChannelName`] = {
        mqtt: {
            ...
            http_publish_funct: async (value, self) => {
                return value ? await shellyHelper.setChannelName(self, `RGBW${rgbwId}`, JSON.parse(value).name) : undefined;
            },
            mqtt_cmd: '<mqttprefix>/rpc',
            mqtt_cmd_funct: (value, self) => {
                return JSON.stringify({
