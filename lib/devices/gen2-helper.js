...
/**
 * Adds a generic Number component definition for Gen 2+ devices
 * see
 * https://shelly-api-docs.shelly.cloud/gen2/DynamicComponents/Virtual/Number#configuration
 *
 * @param deviceObj
 * @param inputId
 */
function addNumber(deviceObj, inputId) {
    deviceObj[`Number${inputId}.ChannelName`] = {
        mqtt: {
            http_publish: `/rpc/Number.GetConfig?id=${inputId}`,
            http_publish_funct: async (value, self) => {
                const channelName = value ? JSON.parse(value).name : undefined;
                if (channelName) {
                    await self.setChannelName(`Number${inputId}`, channelName);
                }
                return channelName;
            },
            mqtt_cmd: '<mqttprefix>/rpc',
    ...
    deviceObj[`Number${inputId}.value`] = {
        mqtt: {
            http_force_status: true, // we must ask via http, as device does not send the value on the start
            http_publish: `/rpc/Number.GetStatus?id=${inputId}`,
            http_publish_funct: value => {
                return value ? parseInt(JSON.parse(value).value) : undefined;
            },
            mqtt_publish: `<mqttprefix>/status/number:${inputId}`,
