...
async function getVoltageMean(self) {
...
let shelly3em = {
  ...
  'Emeter0.Total': {
    ...
    mqtt: {
      ...
      mqtt_publish_funct: (value) => { return (Math.round((value / 1000) * 100) / 100); }
    },
    common: {
      'name': 'Total',
      'type': 'number',
  ...
  'Total.VoltageMean': {
    coap: {
      http_publish: '/status',
      http_publish_funct: async (value, self) => { return getVoltageMean(self); }
    },
    mqtt: {
      http_publish: '/status',
      http_publish_funct: async (value, self) => { return getVoltageMean(self); }
