...
const shellytrv = {
    ...
    'tmp.temperatureTargetF': {
        coap: {
            coap_publish: '3104',
            http_cmd: '/settings/thermostat/0',
            http_cmd_funct: (value) => { return { target_t: shellyHelper.fahrenheitToCelsius(value) }; },
        },
        mqtt: {
            http_publish: '/settings',
            http_publish_funct: (value) => { return value ? shellyHelper.celsiusToFahrenheit(JSON.parse(value).thermostats[0].target_t.value) : undefined; },
            http_cmd: '/settings/thermostat/0',
            http_cmd_funct: (value) => { return { target_t: shellyHelper.fahrenheitToCelsius(value) }; },
        },
        common: {
    ...
    'schedule.profile': {
        coap: {
            ...
            http_cmd: '/settings/thermostat/0',
            http_cmd_funct: (value) => { return { schedule: 1, schedule_profile: value }; },
        },
        mqtt: {
            http_publish: '/status',
            http_publish_funct: (value) => { return value ? JSON.parse(value).thermostats[0].schedule_profile : undefined; },
