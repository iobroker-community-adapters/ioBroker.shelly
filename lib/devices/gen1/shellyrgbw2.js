...
const shellyrgbw2 = {
    ...
    'lights.rgbw': {
        ...
        coap: {
            ...
            http_cmd: '/color/0',
            http_cmd_funct: (value) => { return shellyHelper.getColorsFromRGBW(value); },
        },
        mqtt: {
            mqtt_publish: 'shellies/<mqttprefix>/color/0/status',
            mqtt_publish_funct: async (value, self) => { return await shellyHelper.getRGBW(self) || undefined; },
            mqtt_cmd: 'shellies/<mqttprefix>/color/0/set',
            mqtt_cmd_funct: (value) => { return JSON.stringify(shellyHelper.getColorsFromRGBW(value)); },
        },
        common: {
            name: 'Color RGBW',
            type: 'string',
    ...
    'lights.power': {
        coap: {
            http_publish: '/status',
            http_publish_funct: (value) => { return value ? JSON.parse(value).lights.reduce((pv, cv) => pv + cv.power, 0) : undefined; },
        },
        mqtt: {
            http_publish: '/status',
            http_publish_funct: (value) => { return value ? JSON.parse(value).lights.reduce((pv, cv) => pv + cv.power, 0) : undefined; },
    ...
    'white0.brightness': {
        ...
        mqtt: {
            ...
            mqtt_cmd: 'shellies/<mqttprefix>/white/0/set',
            mqtt_cmd_funct: (value) => { return JSON.stringify({ brightness: value }); },
        },
        common: {
            name: 'Brightness',
            type: 'number',
