...
let shellyswitch = {
  ...
  'Shutter.state': {
    coap: {
      funct_in: (value) => { return value.G[0][2] === 1 ? 'open' : value.G[1][2] === 1 ? 'close' : 'stop'; }
    },
    http: {
      cmd: '/roller/0',
      funct_out: (value) => { return { go: value }; },
    },
    mqtt: {
  ...
  */
  'Shutter.Position': {
    coap: {
      funct_in: (value) => { return value.G[2][2] == -1 ? 101 : value; }
    },
    http: {
      cmd: '/roller/0',
      funct_out: (value) => {
        return { 'go': 'to_pos', 'roller_pos': value };
      }
    },
    mqtt: {
