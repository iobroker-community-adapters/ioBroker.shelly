        ...
        runTestOnEachDevice((deviceClass, device) => {
            ...
            for (const stateId in device) {
                ...
                if (typeof state.common?.name === 'object') {
                    // TODO: Add uk
                    expect(state.common.name, `Missing translation in device class: ${deviceClass} (${stateId})`).to.include.all.keys('en', 'de', 'ru', 'pt', 'nl', 'fr', 'it', 'es', 'pl', 'zh-cn');
                }
            }
        });
    });
    ...
    it('Protocol properties', function () {
        ...
        runTestOnEachDevice((deviceClass, device) => {
            for (const stateId in device) {
                ...
                if (datapoints.getDeviceGen(deviceClass) == 2 || datapoints.getDeviceGen(deviceClass) == 3) {
        ...
        runTestOnEachDevice((deviceClass, device) => {
            for (const stateId in device) {
                ...
                if (Object.prototype.hasOwnProperty.call(validUnitForRoles, state.common.role)) {
                    expect(state.common.unit, `common.unit for role "${state.common.role}" must have unit - ${deviceClass} (${stateId})`).to.be.oneOf(validUnitForRoles[state.common.role]);
                }
            }
        });
    });
