...
describe('Test Device Definitions', function () {
    ...
    it('Device registry - devices must have entries in all lookup tables', function () {
        for (const deviceClass of Object.keys(datapoints.devices)) {
            expect(datapoints.deviceTypes, `Missing deviceTypes entry for ${deviceClass}`).to.have.own.property(
                deviceClass,
            );

            expect(datapoints.deviceGen, `Missing deviceGen entry for ${deviceClass}`).to.have.own.property(deviceClass);
            expect(
                datapoints.deviceGen[deviceClass],
                `Invalid deviceGen value for ${deviceClass}`,
            ).to.be.oneOf([1, 2, 3, 4]);

            expect(
                datapoints.deviceKnowledgeBase,
                `Missing deviceKnowledgeBase entry for ${deviceClass}`,
            ).to.have.own.property(deviceClass);

            expect(datapoints.deviceIcons, `Missing deviceIcons entry for ${deviceClass}`).to.have.own.property(
                deviceClass,
            );

            expect(
                datapoints.deviceGroupMap,
                `Missing deviceGroupMap entry for ${deviceClass}`,
            ).to.have.own.property(deviceClass);
        }
    });

    it('Device registry - lookup tables must not contain unknown devices', function () {
        const deviceKeys = new Set(Object.keys(datapoints.devices));

        for (const deviceClass of Object.keys(datapoints.deviceTypes)) {
            expect(deviceKeys.has(deviceClass), `deviceTypes has unknown device: ${deviceClass}`).to.be.true;
        }

        for (const deviceClass of Object.keys(datapoints.deviceGen)) {
            expect(deviceKeys.has(deviceClass), `deviceGen has unknown device: ${deviceClass}`).to.be.true;
        }

        for (const deviceClass of Object.keys(datapoints.deviceKnowledgeBase)) {
            expect(deviceKeys.has(deviceClass), `deviceKnowledgeBase has unknown device: ${deviceClass}`).to.be.true;
        }

        for (const deviceClass of Object.keys(datapoints.deviceIcons)) {
            expect(deviceKeys.has(deviceClass), `deviceIcons has unknown device: ${deviceClass}`).to.be.true;
        }

        for (const deviceClass of Object.keys(datapoints.deviceGroupMap)) {
            expect(deviceKeys.has(deviceClass), `deviceGroupMap has unknown device: ${deviceClass}`).to.be.true;
        }

        for (const deviceClass of Object.keys(datapoints.pollTime)) {
            expect(deviceKeys.has(deviceClass), `pollTime has unknown device: ${deviceClass}`).to.be.true;
        }
    });

    it('Non-gen1 deviceTypes array must contain only the deviceClass key itself', function () {
        for (const [deviceClass, types] of Object.entries(datapoints.deviceTypes)) {
            if (datapoints.deviceGen[deviceClass] !== 1) {
                expect(
                    types,
                    `deviceTypes for non-gen1 device "${deviceClass}" must be ["${deviceClass}"] but is [${types.map(t => `"${t}"`).join(', ')}]`,
