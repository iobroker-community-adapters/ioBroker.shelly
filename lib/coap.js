...
class CoAPClient {
      ...
      for (let i in all) {
        if (id === i) {
          ...
          for (let j in devices) {
            ...
            let controlFunction;
            if (state.http && state.http.cmd && !state.coap.cmd) {
            ...
            if (state.http && state.http.publish && !state.coap.publish) {
  ...
  createIoBrokerState(payload) {
    for (let i in this.device) {
      ...
      if (dp.coap && dp.coap.funct_in) {
        let value = dp.coap.funct_in(payload);
