...
class CoAPClient {
  ...
  async onlineCheck(stop) {
    ...
    if (stop == 'stop') {
      await this.adapter.setState(deviceId + '.online', { val: false, ack: true });
    } else {
      let val = await this.adapter.getState(deviceId + '.online');
      if (val === false) await this.adapter.setState(deviceId + '.online', { val: true, ack: true });
