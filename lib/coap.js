...
class CoAPClient {
  ...
  getId() {
    ...
    if (devicetype && serialid) return devicetype + '-' + serialid;
  }
   ...
   */
  getDeviceId() {
    let deviceid = datapoints.getDeviceNameForCoAP(this.getDeviceType());
    return deviceid;
  }
  ...
  createObjects() {
    if (Object.keys(this.device).length === 0) {
      let devicetype = datapoints.getDeviceNameForCoAP(this.getDeviceType());
      let devices = datapoints.getDeviceByType(devicetype);
      if (devices) {
