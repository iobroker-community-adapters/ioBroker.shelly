    ...
    for (let i in this.http) {
      ...
      try {
        ...
        for (let j in states) {
          ...
          try {
            let value = state.coap && state.coap.http_publish_funct && isAsync(state.coap.http_publish_funct) ? await state.coap.http_publish_funct(body, this) : state.coap.http_publish_funct(body, this);
            if (!this.states.hasOwnProperty(stateid) || this.states[stateid] !== value) {
              this.states[stateid] = value;
              this.objectHelper.setOrUpdateObject(stateid, {
                type: 'state',
