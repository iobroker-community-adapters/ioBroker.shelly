...
function getCoapValue(objkey, payload) {
  ...
  objkey = isArray ? objkey : Array(objkey.toString());
  let ret = undefined;
  for (let i in objkey) {
    try {
      let key = objkey[i];
...
function getCoapByName(objdescr, sensors, payload) {
  let isArray = (typeof objdescr !== 'string' && typeof objdescr !== 'number');
  objdescr = isArray ? objdescr : Array(objdescr.toString());
  let keys = undefined;
  for (let o in objdescr) {
    let odescr = objdescr[o];
    if (odescr && sensors && sensors.hasOwnProperty(odescr.toLowerCase())) {
      let key = sensors[odescr.toLowerCase()];
      if (isArray) {
        if (keys === undefined) keys = [];
        keys.push(key);
      } else {
        keys = key;
        return getCoapValue(keys, payload);
      }
    }
  }
  if (keys) return getCoapValue(keys, payload);
  return undefined;
}
...
class CoAPClient {
  ...
  async createIoBrokerState(payload) {
    ...
    for (let i in dps) {
      ...
      try {
        if (dp.coap && (dp.coap.coap_publish || dp.coap.coap_publish_funct)) {
          if (dp.coap && dp.coap.coap_publish) {
            value = getCoapValue(dp.coap.coap_publish, value.G);
            // value = getCoapByName('Input Event', this.sensors, payload.G);
          }
