...
(async () => {
  const __mfHostInit = await __mfImport("./hostInit-DQn3OAOo.js");
  await __mfHostInit.__tla;
  const { initHost } = __mfHostInit;
  await initHost();
  const __mfPendingShares = await __mfImport("./pendingShares-CAhrKo3-.js").catch(() => undefined);
...
})().then(() => __mfImport("./index-SWH2d2Y2.js"));
